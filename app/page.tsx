...
function BudgetPage({entries,settings,privateMode,sharedAccountIds,account,memberships,groups,groupBudgetSettings,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset,onClearMoney}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;sharedAccountIds:Set<string>;account:Account;memberships:Membership[];groups:Record<string,Group>;groupBudgetSettings:Record<string,BudgetSettings>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  const now=new Date(),[monthOffset,setMonthOffset]=useState(0),[spaceOpen,setSpaceOpen]=useState(false),[settingsOpen,setSettingsOpen]=useState(false),[monthlyBudgetOpen,setMonthlyBudgetOpen]=useState(false),[transferOpen,setTransferOpen]=useState(false),[editingTransfer,setEditingTransfer]=useState<BudgetEntry|undefined>(undefined),[selectedAccountId,setSelectedAccountId]=useState<string|undefined>(undefined),[editing,setEditing]=useState<BudgetEntry|null|undefined>(undefined),[confirmDelete,setConfirmDelete]=useState<BudgetEntry|null>(null),[lastDeleted,setLastDeleted]=useState<BudgetEntry|null>(null),[search,setSearch]=useState(""),[filter,setFilter]=useState<"all"|"income"|"expense"|"transfer"|"planned">("all"),[view,setView]=useState<"overview"|"recurring">("overview"),[createType,setCreateType]=useState<"income"|"expense">("expense");
  ...
  const upcomingItems=monthEntries.filter(item=>!budgetOccurrencePaid(item.entry,item.occurrenceDate,todayKey)),upcomingExpenses=upcomingItems.filter(item=>item.entry.type==="expense"),upcomingIncomes=upcomingItems.filter(item=>item.entry.type==="income"),upcomingTransfers=upcomingItems.filter(item=>item.entry.type==="transfer"),activeCategoryBudgets=settings.monthlyCategoryBudgets?.[monthKey]||settings.categoryBudgets||{};
  ...
  return <section className="content budget-page">
    ...
    {Object.values(activeCategoryBudgets).some(Boolean)&&<section className="budget-category-progress"><h2>KATEGORIBUDGET · {new Intl.DateTimeFormat("sv-SE",{month:"long"}).format(monthDate).toLocaleUpperCase("sv-SE")}</h2>{Object.entries(activeCategoryBudgets).filter(([,limit])=>limit>0).map(([category,limit])=>{const spent=paidEntries.filter(item=>item.entry.type==="expense"&&item.entry.category===category).reduce((sum,item)=>sum+item.entry.amount,0),percent=Math.min(100,Math.round(spent/limit*100));return <article key={category}><span><b>{budgetCategoryIcons[category]} {category}</b><small>{budgetMoney(spent)} av {budgetMoney(limit)} · {budgetMoney(Math.max(0,limit-spent))} kvar</small></span><progress max="100" value={percent}/>{spent>limit&&<em>Över budget med {budgetMoney(spent-limit)}</em>}</article>})}</section>}
    ...
    {(creating||(editing!==undefined&&editing?.type!=="transfer"))&&<BudgetEditor entry={editing||undefined} initialType={createType} settings={settings} entries={monthEntries.map(item=>item.entry)} monthKey={monthKey} account={account} onClose={()=>{setEditing(undefined);onCreating(false)}} onMonthlyBudget={()=>{setEditing(undefined);onCreating(false);setMonthlyBudgetOpen(true)}} onSave={async entry=>{await onSave(entry);setEditing(undefined);onCreating(false)}} onDelete={editing?()=>setConfirmDelete(editing):undefined}/>} {monthlyBudgetOpen&&<BudgetMonthlyLimitEditor monthKey={monthKey} settings={settings} onClose={()=>setMonthlyBudgetOpen(false)} onSave={async value=>{await onSaveSettings(value);setMonthlyBudgetOpen(false)}}/>} {settingsOpen&&<BudgetSettingsEditor settings={settings} privateMode={privateMode} groups={groups} groupBudgetSettings={groupBudgetSettings} onClose={()=>setSettingsOpen(false)} onSave={async value=>{await onSaveSettings(value);setSettingsOpen(false)}} onReset={async()=>{await onReset();setSettingsOpen(false)}} onClearMoney={async()=>{await onClearMoney();setSettingsOpen(false)}}/>}
...
function BudgetSettingsEditor({settings,privateMode,groups,groupBudgetSettings,onClose,onSave,onReset,onClearMoney}:{settings:BudgetSettings;privateMode:boolean;groups:Record<string,Group>;groupBudgetSettings:Record<string,BudgetSettings>;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  ...
  return <div className="modal-backdrop"><section className="modal budget-settings-editor"><button className="modal-x" onClick={onClose}><X/></button><header><Settings/><h2>BUDGETINSTÄLLNINGAR</h2></header>
    ...
    <footer><button onClick={onClose}>AVBRYT</button><button onClick={()=>{const accountIds=new Set(banks.flatMap(bank=>bank.accounts.map(account=>account.id)));onSave({banks,defaultAccountId:accountIds.has(defaultAccountId)?defaultAccountId:undefined,categoryBudgets,monthlyCategoryBudgets:settings.monthlyCategoryBudgets,savingsGoals:goals.filter(goal=>goal.name.trim()&&goal.target>0),updatedAt:Date.now()})}}>SPARA</button></footer></section></div>
  }

function BudgetMonthlyLimitEditor({monthKey,settings,onClose,onSave}:{monthKey:string;settings:BudgetSettings;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>}){
  const monthName=new Intl.DateTimeFormat("sv-SE",{month:"long",year:"numeric"}).format(new Date(`${monthKey}-01T12:00:00`)),existing=settings.monthlyCategoryBudgets?.[monthKey]||{},[category,setCategory]=useState(budgetCategories.expense[0]),[amount,setAmount]=useState(""),[busy,setBusy]=useState(false),parsedAmount=budgetDecimalNumber(amount);
  useEffect(()=>{const current=existing[category];setAmount(current?String(current).replace(".",","):"")},[category]);
  return <div className="modal-backdrop"><section className="modal budget-monthly-limit-editor"><button className="modal-x" onClick={onClose} aria-label="Stäng"><X/></button><header><span className="budget-choice-icon">◎</span><div><small>BUDGET FÖR MÅNADEN</small><h2>{monthName.toLocaleUpperCase("sv-SE")}</h2></div></header><p className="budget-choice-help">Bestäm hur mycket du vill använda i en kategori. Varje betald utgift dras av och översikten visar hur mycket som är kvar.</p><div className="budget-category-field"><span>KATEGORI</span><BudgetCategoryPicker type="expense" value={category} onChange={setCategory}/></div><label>BELOPP FÖR HELA MÅNADEN<input autoFocus inputMode="decimal" value={amount} onChange={event=>{const next=budgetDecimalText(event.target.value);if(next!==null)setAmount(next)}} placeholder="Till exempel 5 000"/></label>{existing[category]>0&&<small className="budget-existing-limit">Nuvarande budget: {budgetMoney(existing[category])}</small>}<footer><button onClick={onClose}>AVBRYT</button><button disabled={busy||!Number.isFinite(parsedAmount)||parsedAmount<=0} onClick={async()=>{setBusy(true);try{await onSave({...settings,monthlyCategoryBudgets:{...(settings.monthlyCategoryBudgets||{}),[monthKey]:{...existing,[category]:parsedAmount}},updatedAt:Date.now()})}finally{setBusy(false)}}}>{busy?"SPARAR…":"SPARA MÅNADSBUDGET"}</button></footer></section></div>
}
  ...
  return (
    ...
    >
      <section className="modal budget-editor">
        ...
        {status==="planned"&&<label className="budget-recurring budget-auto-pay"><input type="checkbox" checked={autoPay} onChange={event=>setAutoPay(event.target.checked)}/><span><strong>{type==="income"?"Registrera automatiskt på datumet":"Flytta automatiskt till betald på datumet"}</strong><small>{type==="income"?"Inkomsten räknas som mottagen och saldot ändras automatiskt den dagen.":"Utgiften dras från kontot på valt datum. Lämna av om du vill registrera delbetalningar själv."}</small></span></label>}
