      ...
      <aside className={`drawer ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="drawer-footer">
          ...
          <small className="drawer-version-text">
            Bubbsun v0.847 · Web Edition Beta
          </small>
        </div>
      </aside>
    </>
...
function CalendarPage({events,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onOpenList}:{events:CalendarEvent[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete:(event:CalendarEvent)=>Promise<void>;onOpenList:(list:BubbsunList)=>void}){
  ...
  return <section className="content calendar-page">
    ...
    {range==="7"?<div className="calendar-week" onTouchStart={touch=>{if(window.innerWidth<=700){const point=touch.touches[0];weekSwipeStart.current=point?{x:point.clientX,y:point.clientY}:null}}} onTouchEnd={touch=>{const start=weekSwipeStart.current,point=touch.changedTouches[0];weekSwipeStart.current=null;if(!start||!point||window.innerWidth>700)return;const dx=point.clientX-start.x,dy=point.clientY-start.y;if(Math.abs(dx)<55||Math.abs(dx)<Math.abs(dy)*1.25)return;weekSwipeHandled.current=true;setWeekOffset(value=>value+(dx<0?1:-1));window.setTimeout(()=>{weekSwipeHandled.current=false},350)}}>{weekDates.map(date=>{const day=new Date(`${date}T12:00:00`).getDay();return <section key={date} className={`${date===today?"today ":""}${day===0||day===6?"weekend":""}`.trim()}><h2><strong>{new Intl.DateTimeFormat("sv-SE",{weekday:"short"}).format(new Date(`${date}T12:00:00`))}</strong><b>{Number(date.slice(-2))}</b></h2><div onClick={()=>{if(weekSwipeHandled.current)return;setCreateDate(date);onCreating(true)}}>{(grouped[date]||[]).map(event=>eventCard(event,true))}</div></section>})}</div>:!visible.length?<div className="calendar-empty"><CalendarDays/><strong>Inget planerat här</strong><span>Skönt — eller dags att lägga till något?</span><button onClick={()=>{setCreateDate(today);onCreating(true)}}><Plus/> LÄGG TILL</button></div>:<div className="calendar-agenda">{Object.entries(grouped).map(([date,items])=><section key={date}><h2>{calendarDayLabel(date)}</h2>{items.map(event=>eventCard(event))}</section>)}{range==="agenda"&&agendaLimit<visible.length&&<div ref={agendaLoader} className="calendar-loader">LADDAR FLER…</div>}</div>}
