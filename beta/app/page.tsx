    ...
    <section className={`content detail-page ${list.listType==="links"?"links-detail-page":""}`}>
      ...
      <div className={`add-panel ${["wishlist","packing","orders"].includes(list.listType||"") ? "add-panel-stacked-select" : ""} ${list.listType==="cleaning" ? "add-panel-cleaning" : ""} ${list.listType==="home" ? "add-panel-homefix" : ""} ${list.listType==="links" ? "add-panel-links" : ""}`}>
        ...
        <div className={showNewItemNote?"has-add-note":""}>
          ...
          {list.listType==="links"&&<><div className="saved-link-image-choice"><span>{newLinkImage?"Egen bild vald · klicka på bilden för att justera utsnittet":"En stående sidbild hämtas automatiskt"}</span><label className={linkImageBusy?"busy":""}><ImagePlus/>{linkImageBusy?"KOMPRIMERAR…":"VÄLJ EGEN BILD"}<input type="file" accept="image/*" disabled={linkImageBusy} onChange={async event=>{const file=event.target.files?.[0];if(!file)return;setLinkImageBusy(true);try{setNewLinkImage(await compressLinkImage(file));setLinkCropZoom(1);setLinkCropX(0);setLinkCropY(0)}finally{setLinkImageBusy(false)}}}/></label>{newLinkImage&&<button type="button" onClick={()=>setNewLinkImage("")}>ANVÄND AUTOMATISK</button>}</div>{(newLinkImage||cleanLinkUrl(newLinkUrl))?<button type="button" className={`saved-link-preview-button ${newLinkImage?"editable":""}`} disabled={!newLinkImage} onClick={()=>newLinkImage&&setLinkCropOpen(true)} aria-label={newLinkImage?"Justera bildens utsnitt":"Automatisk förhandsvisning"}><img className="saved-link-new-preview" src={newLinkImage||automaticLinkImage(cleanLinkUrl(newLinkUrl))} alt="Förhandsvisning av sidan"/></button>:<div className="saved-link-empty-preview"><ImagePlus/><span>INGEN BILD ÄN</span></div>}</>}
