...
function RecipeShareControl({recipe}:{recipe:Recipe}){
  ...
  return <><button className="recipe-share-button" type="button" onClick={()=>setOpen(true)}><Share2/> DELA</button>{open&&<div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}><div className="modal share-list-modal recipe-share-modal"><button className="modal-close" aria-label="Stäng" onClick={()=>setOpen(false)}><X/></button><Share2 className="share-modal-icon"/><h2>DELA RECEPT</h2><p>Alla med länken kan läsa det här publika receptet utan konto.</p><input readOnly value={url} onFocus={event=>event.currentTarget.select()}/><div className="share-site-actions"><button onClick={async()=>{await navigator.clipboard.writeText(url);window.alert("Länken är kopierad ✓")}}><Copy/> KOPIERA LÄNK</button>{typeof navigator.share==="function"&&<button onClick={()=>void navigator.share({title:recipe.title,text:`${recipe.title} – ett recept på Bubbsun`,url})}><Share2/> DELA</button>}</div></div></div>}</>;
}
...
function RecipesPage({recipes,lists,privateMode,account,memberships,groups,members,onMode,onSwitchGroup,onSave,onDelete,onAddToList,openRecipeId,onRecipeOpened}:{recipes:Recipe[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>Promise<void>;onSave:(recipe:Recipe)=>Promise<void>;onDelete:(recipe:Recipe)=>Promise<void>;onAddToList:(recipe:Recipe,listId:string)=>Promise<void>;openRecipeId?:string;onRecipeOpened?:()=>void}){
  const [creating,setCreating]=useState(false),[editing,setEditing]=useState<Recipe|undefined>(),[viewing,setViewing]=useState<Recipe|undefined>(),[search,setSearch]=useState(""),[category,setCategory]=useState(""),[subcategory,setSubcategory]=useState(""),[spaceOpen,setSpaceOpen]=useState(false);const activeGroup=groups[account.activeGroupId];
  ...
  return <section className="content recipes-page">
    ...
    <div className="recipes-toolbar recipe-discover-toolbar recipe-cookbook-toolbar"><label><Search/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Sök recept eller skapare"/></label><RecipeFilterPicker label="KATEGORI" value={category} options={categoryOptions} onChange={value=>{setCategory(value);setSubcategory("")}}/><RecipeFilterPicker label="UNDERKATEGORI" value={subcategory} options={subcategoryOptions} disabled={!category} onChange={setSubcategory}/><button onClick={()=>setCreating(true)}><Plus/> NYTT RECEPT</button></div>
    {shown.length?<div className="recipe-grid">{shown.map(recipe=><button className={`recipe-card${recipe.isPublic?" shared":""}`} key={recipe.id} style={{"--recipe-creator":rgbaHex(creatorColor(recipe))} as CSSProperties} onClick={()=>setViewing(recipe)}>{recipe.image?<img src={recipe.image} alt=""/>:<span className="recipe-fallback">🍲</span>}<span><small>{recipeCategoryLabel(recipe)}</small><strong>{recipe.title}</strong><em>{recipe.minutes?`${recipe.minutes} min · `:""}{recipe.servings} portioner</em></span></button>)}</div>:<div className="recipes-empty"><span>🥐</span><h2>Inga recept här ännu</h2><p>Börja med något gott. Bullar är ett stabilt förstaval.</p><button onClick={()=>setCreating(true)}>SKAPA RECEPT</button></div>}
    ...
    {viewing?.isPublic&&<div className="recipe-own-share-floating"><RecipeShareControl recipe={viewing}/></div>}
    {(creating||editing)&&<RecipeEditor recipe={editing} account={account} lists={lists} onClose={()=>{setCreating(false);setEditing(undefined)}} onSave={async recipe=>{await onSave(recipe);setCreating(false);setEditing(undefined)}} onDelete={editing?async()=>{await onDelete(editing);setEditing(undefined)}:undefined}/>}</section>;
}
...
function DiscoverRecipesPage({recipes}:{recipes:Recipe[]}){
  const [search,setSearch]=useState(""),[category,setCategory]=useState(""),[subcategory,setSubcategory]=useState(""),[viewing,setViewing]=useState<Recipe|undefined>();
  const categoryOptions=[{value:"",label:"Alla kategorier",count:recipes.length},...recipeCategories.filter(Boolean).map(value=>({value,label:value,count:recipes.filter(recipe=>recipe.category===value).length}))],categoryRecipes=category?recipes.filter(recipe=>recipe.category===category):recipes,availableSubcategories=category?Array.from(new Set([...(recipeSubcategories[category]||[]),...categoryRecipes.map(recipe=>recipe.subcategory||"").filter(Boolean)])):[],subcategoryOptions=[{value:"",label:category?"Alla underkategorier":"Välj kategori först",count:categoryRecipes.length},...availableSubcategories.map(value=>({value,label:value,count:categoryRecipes.filter(recipe=>recipe.subcategory===value).length}))];
  const shown=recipes.filter(recipe=>(!category||recipe.category===category)&&(!subcategory||recipe.subcategory===subcategory)&&(!search.trim()||`${recipe.title} ${recipe.creatorName} ${recipe.category} ${recipe.subcategory||""}`.toLocaleLowerCase("sv-SE").includes(search.trim().toLocaleLowerCase("sv-SE"))));
  return <section className="content recipes-page recipe-discover-page">
    ...
    {shown.length?<div className="recipe-grid">{shown.map(recipe=><button className="recipe-card public" key={`${recipe.creatorId}-${recipe.id}`} style={{"--recipe-creator":"var(--theme-accent)"} as CSSProperties} onClick={()=>setViewing(recipe)}>{recipe.image?<img src={recipe.image} alt=""/>:<span className="recipe-fallback">🍲</span>}<span><small>{recipeCategoryLabel(recipe)}</small><strong>{recipe.title}</strong><em>{recipe.minutes?`${recipe.minutes} min · `:""}{recipe.servings} portioner · av {recipe.creatorName}</em></span></button>)}</div>:<div className="recipes-empty"><span>🔎</span><h2>Inga recept hittades</h2><p>Här dyker offentliga recept upp när någon delar ett.</p></div>}
    {viewing&&<div className="recipe-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setViewing(undefined)}}><article className="recipe-view" style={{"--recipe-creator":"var(--theme-accent)"} as CSSProperties}><button className="recipe-close" onClick={()=>setViewing(undefined)}><X/></button>{viewing.image?<img className="recipe-hero" src={viewing.image} alt=""/>:<div className="recipe-hero fallback">🍲</div>}<small>{recipeCategoryLabel(viewing)}</small><h1>{viewing.title}</h1><div className="recipe-facts"><span>🍽️ {viewing.servings} portioner</span>{viewing.minutes>0&&<span>⏱️ {viewing.minutes} minuter</span>}</div><div className="recipe-creator-line">Skapad av {viewing.creatorName}</div><section><h3>INGREDIENSER</h3><ul>{viewing.ingredients.map(item=><li key={item.id}><b>{[item.amount,item.unit].filter(Boolean).join(" ")}</b> {item.name}</li>)}</ul></section><section><h3>GÖR SÅ HÄR</h3><div className="recipe-instructions">{recipeInstructionSteps(viewing.instructions).map((step,index)=><p key={index}><b>{index+1}</b><span>{step}</span></p>)}</div></section>{viewing.note&&<aside><b>ANTECKNING</b><p>{viewing.note}</p></aside>}<footer><span/><div><RecipeShareControl recipe={viewing}/></div></footer></article></div>}
  </section>;
}
...
function PublicRecipeArticle({recipe}:{recipe:Recipe}){return <article className="recipe-view public-recipe-view" style={{"--recipe-creator":"var(--theme-accent)"} as CSSProperties}>{recipe.image?<img className="recipe-hero" src={recipe.image} alt={recipe.title}/>:<div className="recipe-hero fallback">🍲</div>}<small>{recipeCategoryLabel(recipe)}</small><h1>{recipe.title}</h1><div className="recipe-facts"><span>🍽️ {recipe.servings} portioner</span>{recipe.minutes>0&&<span>⏱️ {recipe.minutes} minuter</span>}</div><div className="recipe-creator-line">Skapad av {recipe.creatorName}</div><section><h3>INGREDIENSER</h3><ul>{recipe.ingredients.map(item=><li key={item.id}><b>{[item.amount,item.unit].filter(Boolean).join(" ")}</b> {item.name}</li>)}</ul></section><section><h3>GÖR SÅ HÄR</h3><div className="recipe-instructions">{recipeInstructionSteps(recipe.instructions).map((step,index)=><p key={index}><b>{index+1}</b><span>{step}</span></p>)}</div></section>{recipe.note&&<aside><b>ANTECKNING</b><p>{recipe.note}</p></aside>}<footer><a href="/recept">← Upptäck fler recept</a><div><RecipeShareControl recipe={recipe}/></div></footer></article>}
...
function PublicRecipePage({recipeId}:{recipeId:string}){
  ...
  return <main className="public-recipe-page"><a className="public-recipe-brand" href="/">Bubbsun<span>.se</span></a><PublicRecipeArticle recipe={recipe}/></main>;
}
...
function PublicRecipesIndexPage(){
  ...
  return <main className="public-recipes-index"><a className="public-recipe-brand" href="/">Bubbsun<span>.se</span></a><header><BookOpen/><div><small>BUBBSUNS RECEPT</small><h1>Upptäck något gott</h1><p>Publika recept från Bubbsun-användare. Inget konto behövs.</p></div></header>{!ready?<p>Laddar recept…</p>:recipes.length?<div className="recipe-grid">{recipes.map(recipe=><a className="recipe-card public" href={`/recept/${encodeURIComponent(recipe.id)}/${recipeSlug(recipe.title)}`} key={recipe.id}>{recipe.image?<img src={recipe.image} alt={recipe.title}/>:<span className="recipe-fallback">🍲</span>}<span><small>{recipeCategoryLabel(recipe)}</small><strong>{recipe.title}</strong><em>{recipe.minutes?`${recipe.minutes} min · `:""}{recipe.servings} portioner · av {recipe.creatorName}</em></span></a>)}</div>:<p>Inga publika recept ännu.</p>}</main>;
}
