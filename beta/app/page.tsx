...
const automaticLinkImage=(url:string)=>url?`https://image.thum.io/get/width/700/crop/990/noanimate/${url}`:"";
...
const importLinkImage=async(url:string)=>{const response=await fetch(url,{mode:"cors"});if(!response.ok)throw new Error("Bildtjänsten svarade inte");const blob=await response.blob();return compressLinkImage(new File([blob],"sidbild",{type:blob.type||"image/jpeg"}))};
const cropLinkImage=(source:string,zoom:number,offsetX:number,offsetY:number)=>new Promise<string>((resolve,reject)=>{const image=new Image();image.onload=()=>{const width=420,height=594,canvas=document.createElement("canvas"),context=canvas.getContext("2d");canvas.width=width;canvas.height=height;if(!context){reject(new Error("Bilden kunde inte redigeras"));return}context.fillStyle="#fff8e9";context.fillRect(0,0,width,height);const scale=Math.min(width/image.width,height/image.height)*zoom,renderWidth=image.width*scale,renderHeight=image.height*scale,maxShiftX=Math.max(0,(renderWidth-width)/2),maxShiftY=Math.max(0,(renderHeight-height)/2),drawX=(width-renderWidth)/2+(offsetX/100)*maxShiftX,drawY=(height-renderHeight)/2+(offsetY/100)*maxShiftY;context.drawImage(image,drawX,drawY,renderWidth,renderHeight);let quality=.58,result=canvas.toDataURL("image/webp",quality);while(result.length>70000&&quality>.25){quality-=.07;result=canvas.toDataURL("image/webp",quality)}resolve(result)};image.onerror=()=>reject(new Error("Bilden kunde inte redigeras"));image.src=source});
function LinkImageCropModal({source,onCancel,onApply}:{source:string;onCancel:()=>void;onApply:(image:string)=>void}){const [zoom,setZoom]=useState(1),[position,setPosition]=useState({x:0,y:0}),[busy,setBusy]=useState(false),drag=useRef<{x:number;y:number;pointerId:number}|null>(null);const clamp=(value:number)=>Math.max(-100,Math.min(100,value)),stopDrag=()=>{drag.current=null},travel=(zoom-1)/(zoom*2);if(typeof document==="undefined")return null;const shell=document.querySelector<HTMLElement>(".app-shell"),computed=shell?getComputedStyle(shell):null,portalStyle=Object.fromEntries(["--theme-panel","--theme-paper","--theme-text","--theme-outline","--theme-accent"].map(key=>[key,computed?.getPropertyValue(key)||""])) as CSSProperties;return createPortal(<div className="modal-backdrop link-crop-backdrop" style={portalStyle}><div className="link-crop-modal"><h2>JUSTERA BILDEN</h2><div className="link-crop-viewport" onDragStart={event=>event.preventDefault()} onPointerDown={event=>{event.preventDefault();drag.current={x:event.clientX,y:event.clientY,pointerId:event.pointerId};event.currentTarget.setPointerCapture(event.pointerId)}} onPointerMove={event=>{const start=drag.current;if(!start||start.pointerId!==event.pointerId)return;event.preventDefault();const box=event.currentTarget.getBoundingClientRect(),dx=event.clientX-start.x,dy=event.clientY-start.y;start.x=event.clientX;start.y=event.clientY;setPosition(value=>({x:clamp(value.x+(dx/box.width)*140/zoom),y:clamp(value.y+(dy/box.height)*140/zoom)}))}} onPointerUp={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);stopDrag()}} onPointerCancel={stopDrag} onLostPointerCapture={stopDrag}><img draggable={false} src={source} alt="Bildutsnitt" style={{transform:`scale(${zoom}) translate(${position.x*travel}%,${position.y*travel}%)`}}/></div><p className="link-crop-hint">Ta tag i bilden och dra den till rätt läge.</p><label>ZOOM<input type="range" min="1" max="3" step="0.05" value={zoom} onChange={event=>setZoom(Number(event.target.value))}/></label><div><button className="cancel" disabled={busy} onClick={onCancel}>AVBRYT</button><button disabled={busy} onClick={async()=>{setBusy(true);try{onApply(await cropLinkImage(source,zoom,position.x,position.y))}finally{setBusy(false)}}}>{busy?"SPARAR…":"ANVÄND UTSNITT"}</button></div></div></div>,document.body)}
      ...
      <div className="topbar-inner">
        ...
        {gameXp ? <div className="header-game-xp"><Star/><span><small>NIVÅ {gameXp.level}</small><strong>{gameXp.xp.toLocaleString("sv-SE")} XP</strong><i><em style={{width:`${Math.min(100,gameXp.levelXp/Math.max(1,gameXp.nextLevelXp)*100)}%`}}/></i></span></div> : wallet ? <div className="header-game-wallet"><span><small>SALDO</small><strong>{wallet.balance.toLocaleString("sv-SE")} Bb</strong></span><span><small>VALV</small><strong>{wallet.vault.toLocaleString("sv-SE")} Bb</strong></span></div> : mode === "calculator" ? <div className="header-add-actions"><button className="theme-button header-add header-calculator" aria-label="Öppna miniräknaren" title="Miniräknare" onClick={onAdd}><Calculator/></button></div> : mode === "add" ? (
          <div className="header-add-actions"><button
            ...
            aria-label={addIcon === "note" ? "Skapa ett memo" : addIcon === "recipe" ? "Skapa ett recept" : addIcon === "calendar" ? "Ny händelse" : "Skapa en lista"}
            onClick={onAdd}
          >
...
function NoteFollowButton({note,uid,groupId}:{note:BubbsunNote;uid:string;groupId:string}){
  ...
  return <button type="button" className="note-follow-tool" onClick={()=>void toggle()}><Bell/><span><strong>{following?"SLUTA FÖLJA":"FÖLJ MEMO"}</strong><small>Få en notis vid ändringar</small></span><ChevronRight/></button>;
}

function GroupSpaceTab({privateMode,uid,groupName,groupIconId,activeGroupId,memberships,groups,onMode,onSwitch}:{privateMode:boolean;uid:string;groupName:string;groupIconId?:string;activeGroupId:string;memberships:Membership[];groups:Record<string,Group>;onMode:(value:boolean)=>void;onSwitch:(groupId:string)=>void}){
  const [open,setOpen]=useState(false);
  const [counts,setCounts]=useState<Record<string,{lists:number;notes:number}>>({});
  const [privateCounts,setPrivateCounts]=useState({lists:0,notes:0});
  ...
  useEffect(()=>{
    if(!open)return;
    const unsubs=[watchPrivateLists(uid,items=>setPrivateCounts(current=>({...current,lists:items.length}))),watchPrivateNotes(uid,items=>setPrivateCounts(current=>({...current,notes:items.length}))),...memberships.flatMap(membership=>[watchLists(membership.groupId,items=>setCounts(current=>({...current,[membership.groupId]:{lists:items.length,notes:current[membership.groupId]?.notes??0}}))),watchNotes(membership.groupId,items=>setCounts(current=>({...current,[membership.groupId]:{lists:current[membership.groupId]?.lists??0,notes:items.length}})))])];
    return()=>unsubs.forEach(unsubscribe=>unsubscribe());
  },[open,uid,memberships.map(value=>value.groupId).join("|")]);
  return <div ref={root} className={`group-space-tab has-menu ${privateMode?"private-space":"selected"} ${open?"menu-open":""}`}>
    <button type="button" className="group-space-main" aria-label="Välj privat eller grupp" aria-expanded={open} onClick={()=>setOpen(value=>!value)}>{privateMode?<LockKeyhole/>:groupIconId?<GroupIcon id={groupIconId}/>:<Users/>}<span>{privateMode?"PRIVAT":"GRUPP"}<small>{privateMode?"Bara jag":groupName}</small></span></button>
    <button type="button" className="group-space-trigger" aria-label="Välj privat eller grupp" aria-expanded={open} onClick={()=>setOpen(value=>!value)}><ChevronDown className={open?"turn":""}/></button>
    {open&&<div className="group-space-menu" role="menu"><button type="button" role="menuitem" className={privateMode?"selected":""} onClick={()=>{setOpen(false);onMode(true)}}><span className="group-space-menu-icon"><LockKeyhole/></span><strong>Privat</strong><span className="group-space-menu-counts"><span title={`${privateCounts.lists} listor`}><ListChecks/><b>{privateCounts.lists}</b></span><span title={`${privateCounts.notes} memo`}><NotebookPen/><b>{privateCounts.notes}</b></span></span>{privateMode&&<Check/>}</button>{memberships.map(membership=>{const group=groups[membership.groupId],count=counts[membership.groupId]??{lists:0,notes:0},active=!privateMode&&membership.groupId===activeGroupId;return <button type="button" role="menuitem" key={membership.groupId} className={active?"selected":""} onClick={()=>{setOpen(false);if(membership.groupId===activeGroupId)onMode(false);else onSwitch(membership.groupId)}}><span className="group-space-menu-icon">{group?.iconId?<GroupIcon id={group.iconId}/>:<Users/>}</span><strong>{group?.name||"Grupp"}</strong><span className="group-space-menu-counts"><span title={`${count.lists} listor`}><ListChecks/><b>{count.lists}</b></span><span title={`${count.notes} memo`}><NotebookPen/><b>{count.notes}</b></span></span>{active&&<Check/>}</button>})}{!memberships.length&&<button type="button" className="group-empty-state" onClick={()=>{setOpen(false);window.dispatchEvent(new CustomEvent("bubbsun:navigate",{detail:"people"}))}}><Users/><span><strong>Ingen grupp ännu</strong><small>Skapa en grupp eller gå med i en på sidan Användare &amp; grupper.</small><b>TILL ANVÄNDARE &amp; GRUPPER</b></span><ChevronRight/></button>}</div>}
  </div>
}

function NotesPage({notes,uid,privateMode,groupName,groupIconId,activeGroupId,memberships,groups,resolveCreatorColor,followedNoteIds,onMode,onSwitchGroup,onLists,onHelp,onOpen,onReorder}:{notes:BubbsunNote[];uid:string;privateMode:boolean;groupName:string;groupIconId?:string;activeGroupId:string;memberships:Membership[];groups:Record<string,Group>;resolveCreatorColor:(note:BubbsunNote)=>number;followedNoteIds:Set<string>;onMode:(value:boolean)=>void;onSwitchGroup:(groupId:string)=>void;onLists:()=>void;onHelp:()=>void;onOpen:(note:BubbsunNote)=>void;onReorder:(from:number,to:number)=>void}){
  ...
  return <section className="content list-page notes-page"><div className="list-notes-controls"><button className="list-view-tab" onClick={onLists}><ListChecks/> LISTOR</button><GroupSpaceTab privateMode={privateMode} uid={uid} groupName={groupName} groupIconId={groupIconId} activeGroupId={activeGroupId} memberships={memberships} groups={groups} onMode={onMode} onSwitch={onSwitchGroup}/><span className="list-filter-placeholder"/><button className="notes-view-tab selected"><NotebookPen/> MEMO</button></div>{!notes.length&&<div className="empty-card"><NotebookPen/><strong>{privateMode?"Här är tomt än så länge":"Gruppen har inga memo än"}</strong><span>{privateMode?"Skapa ditt första memo med plusknappen uppe till höger.":"Skapa gruppens första memo med plusknappen uppe till höger."}</span><button type="button" onClick={onHelp}><span>👋</span><span><b>NY HÄR?</b><small>Här kan du skriva idéer, planer och sådant du vill minnas.</small></span><ChevronRight/></button></div>}<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={end}><SortableContext items={notes.map(note=>note.id)} strategy={rectSortingStrategy}><div className="list-stack">{notes.map(note=><SortableNoteCard key={note.id} note={note} creatorColor={resolveCreatorColor(note)} followed={!privateMode&&followedNoteIds.has(note.id)} onOpen={()=>onOpen(note)}/>)}</div></SortableContext></DndContext></section>
}
...
function NoteEditorPage({note,onSave,onBack,onDelete,follow,creator,toolsOpen,onToolsOpen}:{note:BubbsunNote;onSave:(note:BubbsunNote)=>Promise<boolean>;onBack:()=>void;onDelete:()=>void;follow?:{uid:string;groupId:string};creator?:{name:string;color:number};toolsOpen:boolean;onToolsOpen:(open:boolean)=>void}){
  ...
  return <section className="content note-editor-page">
    ...
    <article>
      ...
      <label className="note-body-label" htmlFor="note-body">MEMO</label>
      <textarea id="note-body" value={text} onChange={event=>setText(event.target.value)} onKeyDown={handleTab} placeholder="Skriv ditt memo här…"/>
    ...
    {confirmDelete&&<div className="modal-backdrop"><div className="modal confirm-delete-modal"><h2>TA BORT MEMOT?</h2><p>Memot “{note.title}” försvinner.</p><div className="modal-actions"><button className="cancel" onClick={()=>setConfirmDelete(false)}>AVBRYT</button><button className="danger" onClick={()=>onDelete()}>TA BORT</button></div></div></div>}
...
function NewNoteEditor({onCancel,onSave}:{onCancel:()=>void;onSave:(note:BubbsunNote)=>void}){const [title,setTitle]=useState(""),[icon,setIcon]=useState("idea"),[color,setColor]=useState(listColorOptions[0]);return <div className="modal-backdrop"><form className="modal note-new-editor" onSubmit={event=>{event.preventDefault();if(title.trim())onSave({id:crypto.randomUUID(),title:title.trim(),text:"",icon,color,order:Date.now(),creatorId:""})}}><h2>NYTT MEMO</h2><input autoFocus value={title} onChange={event=>setTitle(event.target.value)} placeholder="Vad handlar den om?"/><NoteAppearancePicker icon={icon} color={color} onIcon={setIcon} onColor={setColor}/><div className="modal-actions"><button type="button" className="cancel" onClick={onCancel}>AVBRYT</button><button type="submit">SKAPA</button></div></form></div>}
...
function ListsPage({
  ...
  return (
    <section className="content list-page">
      <div className="list-notes-controls">
        <button type="button" className="list-view-tab selected"><ListChecks /> LISTOR</button>
        <GroupSpaceTab privateMode={privateMode} uid={uid} groupName={groupName} groupIconId={groupIconId} activeGroupId={activeGroupId} memberships={memberships} groups={groups} onMode={onMode} onSwitch={onSwitchGroup}/>
        <div className="desktop-list-tab list-category-slot">
          <button
            type="button"
        ...
        <button className="notes-view-tab" onClick={onNotes}><NotebookPen /> MEMO</button>
      </div>
...
function SortableItemRow({
  ...
  const [draftLinkCropSource,setDraftLinkCropSource]=useState("");
  ...
  return (
    ...
    >
      ...
      <button className="item-copy" onClick={()=>!selecting&&onRequestExpand()} aria-expanded={expanded}>
        ...
        <span className="item-title-line">{item.note&&<NotebookPen className="item-note-marker" aria-label="Har memo" />}<strong>{item.name}</strong>{isNew&&<em className="new-badge item-new-badge">NYTT</em>}</span>
    ...
    {expanded&&!selecting&&<div id={`item-editor-${item.id}`} className={`item-inline-editor ${flashUnsaved?"flash-unsaved":""} ${listType==="links"?"saved-link-editor":""}`}>
      ...
      <label className="item-note-field">Memo (valfritt)<textarea value={draftNote} onChange={event=>{setDraftNote(event.target.value);onDirtyChange(true)}} placeholder="Skriv ett memo…" /></label>
      {listType==="links"&&<><label>Länk<input type="url" inputMode="url" value={draftLinkUrl} onChange={event=>{setDraftLinkUrl(event.target.value);onDirtyChange(true)}} placeholder="https://…" /></label>{cleanLinkUrl(draftLinkUrl)&&<a className="saved-link-editor-open" href={cleanLinkUrl(draftLinkUrl)} target="_blank" rel="noreferrer" aria-label="Öppna länken"><ExternalLink/></a>}<div className="saved-link-preview-column"><strong>Förhandsvisning</strong>{(draftLinkImage||cleanLinkUrl(draftLinkUrl))&&<button type="button" className="saved-link-edit-preview-button" onClick={async()=>{if(draftLinkImage){setDraftLinkCropSource(draftLinkImage);return}try{const imported=await importLinkImage(automaticLinkImage(cleanLinkUrl(draftLinkUrl)));setDraftLinkImage(imported);setDraftLinkCropSource(imported);onDirtyChange(true)}catch{window.alert("Sidbilden kunde inte öppnas för redigering.")}}}><img className="saved-link-edit-preview" src={draftLinkImage||automaticLinkImage(cleanLinkUrl(draftLinkUrl))} alt="Förhandsvisning"/></button>}<label className="saved-link-upload"><ImagePlus/> Byt bild<input type="file" accept="image/*" onChange={async event=>{const file=event.target.files?.[0];if(file){setDraftLinkImage(await compressLinkImage(file));onDirtyChange(true)}}}/></label></div></>}
    ...
    {draftLinkCropSource&&<LinkImageCropModal source={draftLinkCropSource} onCancel={()=>setDraftLinkCropSource("")} onApply={image=>{setDraftLinkImage(image);setDraftLinkCropSource("");onDirtyChange(true)}}/>}
    </div>
  );
}
...
function ListPage({
  ...
  const [linkCropOpen,setLinkCropOpen]=useState(false);
  ...
  return (
    <section className={`content detail-page ${list.listType==="links"?"links-detail-page":""}`}>
      ...
      {printOpen&&<div className="modal-backdrop"><div className="modal print-list-modal">
        ...
        <label className="share-notes-choice"><input type="checkbox" checked={printNotes} onChange={event=>setPrintNotes(event.target.checked)}/><span><strong>Ta med memo</strong><small>Avstängt som standard</small></span></label>
        <button onClick={()=>{
      ...
      <div className={`add-panel ${["wishlist","packing","orders"].includes(list.listType||"") ? "add-panel-stacked-select" : ""} ${list.listType==="cleaning" ? "add-panel-cleaning" : ""} ${list.listType==="home" ? "add-panel-homefix" : ""} ${list.listType==="links" ? "add-panel-links" : ""}`}>
        ...
        <div className={showNewItemNote?"has-add-note":""}>
          ...
          />}
          {list.listType==="links"&&<input className="saved-link-url-input" type="url" inputMode="url" value={newLinkUrl} onChange={event=>setNewLinkUrl(event.target.value)} placeholder="Klistra in länken här…"/>}
          ...
          {list.listType==="links"&&<><div className="saved-link-image-choice"><span>{newLinkImage?"Sparad bild · klicka på bilden för att justera utsnittet":"Sidbild hämtad · klicka på bilden för att justera utsnittet"}</span><label className={linkImageBusy?"busy":""}><ImagePlus/>{linkImageBusy?"KOMPRIMERAR…":"VÄLJ EGEN BILD"}<input type="file" accept="image/*" disabled={linkImageBusy} onChange={async event=>{const file=event.target.files?.[0];if(!file)return;setLinkImageBusy(true);try{setNewLinkImage(await compressLinkImage(file))}finally{setLinkImageBusy(false)}}}/></label>{newLinkImage&&<button type="button" onClick={()=>setNewLinkImage("")}>ANVÄND AUTOMATISK</button>}</div>{(newLinkImage||cleanLinkUrl(newLinkUrl))?<button type="button" className="saved-link-preview-button editable" disabled={linkImageBusy} onClick={async()=>{if(newLinkImage){setLinkCropOpen(true);return}const pageUrl=cleanLinkUrl(newLinkUrl);if(!pageUrl)return;setLinkImageBusy(true);try{setNewLinkImage(await importLinkImage(automaticLinkImage(pageUrl)));setLinkCropOpen(true)}catch{window.alert("Sidbilden kunde inte öppnas för redigering. Du kan fortfarande välja en egen bild.")}finally{setLinkImageBusy(false)}}} aria-label="Justera bildens utsnitt"><img className="saved-link-new-preview" src={newLinkImage||automaticLinkImage(cleanLinkUrl(newLinkUrl))} alt="Förhandsvisning av sidan"/></button>:<div className="saved-link-empty-preview"><ImagePlus/><span>INGEN BILD ÄN</span></div>}</>}
          ...
          {list.listType!=="links"&&showNewItemNote&&<textarea className="new-item-note" value={newItemNote} onChange={event=>setNewItemNote(event.target.value)} placeholder="Skriv ett memo (valfritt)…" autoFocus/>}
          <span className="add-panel-actions">
            {list.listType!=="links"&&<button type="button" className={showNewItemNote?"active":""} aria-label={showNewItemNote?"Dölj memo":"Lägg till memo"} aria-pressed={showNewItemNote} onClick={()=>setShowNewItemNote(value=>!value)}><NotebookPen/></button>}
            <button type="button" aria-label="Lägg till" onClick={() => add()}><Check /></button>
          </span>
        </div>
      </div>
      {linkCropOpen&&newLinkImage&&<LinkImageCropModal source={newLinkImage} onCancel={()=>setLinkCropOpen(false)} onApply={image=>{setNewLinkImage(image);setLinkCropOpen(false)}}/>}
      {selecting && (
        <div
...
function BudgetPage({entries,settings,privateMode,sharedAccountIds,account,memberships,groups,groupBudgetSettings,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset,onClearMoney}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;sharedAccountIds:Set<string>;account:Account;memberships:Membership[];groups:Record<string,Group>;groupBudgetSettings:Record<string,BudgetSettings>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  const now=new Date(),[monthOffset,setMonthOffset]=useState(0),[spaceOpen,setSpaceOpen]=useState(false),[settingsOpen,setSettingsOpen]=useState(false),[settingsFocus,setSettingsFocus]=useState<"categories"|null>(null),[transferOpen,setTransferOpen]=useState(false),[editingTransfer,setEditingTransfer]=useState<BudgetEntry|undefined>(undefined),[selectedAccountId,setSelectedAccountId]=useState<string|undefined>(undefined),[editing,setEditing]=useState<BudgetEntry|null|undefined>(undefined),[confirmDelete,setConfirmDelete]=useState<BudgetEntry|null>(null),[lastDeleted,setLastDeleted]=useState<BudgetEntry|null>(null),[search,setSearch]=useState(""),[filter,setFilter]=useState<"all"|"income"|"expense"|"transfer"|"planned">("all"),[view,setView]=useState<"overview"|"recurring">("overview"),[createType,setCreateType]=useState<"income"|"expense">("expense");
  ...
  return <section className="content budget-page">
    ...
    {(creating||(editing!==undefined&&editing?.type!=="transfer"))&&<BudgetEditor entry={editing||undefined} initialType={createType} settings={settings} entries={monthEntries.map(item=>item.entry)} monthKey={monthKey} account={account} onClose={()=>{setEditing(undefined);onCreating(false)}} onMonthlyBudget={()=>{setEditing(undefined);onCreating(false);setSettingsFocus("categories");setSettingsOpen(true)}} onSave={async entry=>{await onSave(entry);setEditing(undefined);onCreating(false)}} onDelete={editing?()=>setConfirmDelete(editing):undefined}/>} {settingsOpen&&<BudgetSettingsEditor settings={settings} privateMode={privateMode} groups={groups} groupBudgetSettings={groupBudgetSettings} focusCategoryBudgets={settingsFocus==="categories"} onClose={()=>{setSettingsOpen(false);setSettingsFocus(null)}} onSave={async value=>{await onSaveSettings(value);setSettingsOpen(false);setSettingsFocus(null)}} onReset={async()=>{await onReset();setSettingsOpen(false);setSettingsFocus(null)}} onClearMoney={async()=>{await onClearMoney();setSettingsOpen(false);setSettingsFocus(null)}}/>}
...
function BudgetCalculator({onClose}:{onClose:()=>void}){
  const saved=useMemo(()=>{try{const value=JSON.parse(localStorage.getItem("bubbsun-budget-calculator")||"null") as Record<string,unknown>|null,operator=value&&["+","−","×","÷"].includes(String(value.operator))?value.operator as "+"|"−"|"×"|"÷":null;return{display:typeof value?.display==="string"?value.display:"0",formula:typeof value?.formula==="string"?value.formula:"",stored:typeof value?.stored==="number"?value.stored:null,operator,fresh:value?.fresh!==false}}catch{return{display:"0",formula:"",stored:null,operator:null,fresh:true}}},[]),[display,setDisplay]=useState(saved.display),[formula,setFormula]=useState(saved.formula),[stored,setStored]=useState<number|null>(saved.stored),[operator,setOperator]=useState<"+"|"−"|"×"|"÷"|null>(saved.operator),[fresh,setFresh]=useState(saved.fresh);
  useEffect(()=>{try{localStorage.setItem("bubbsun-budget-calculator",JSON.stringify({display,formula,stored,operator,fresh}))}catch{}},[display,formula,stored,operator,fresh]);
  ...
  const digit=(value:string)=>{if(display==="Fel"||fresh){if(formula&&stored===null&&!operator)setFormula("");setDisplay(value);setFresh(false)}else if(display.replace(/[^0-9]/g,"").length<12)setDisplay(display==="0"?value:display+value)};
  const choose=(next:typeof operator)=>{if(fresh&&formula){setFormula(`${formula.slice(0,-1)}${next}`);setOperator(next);return}const current=numberValue(),base=stored!==null&&operator?calculate(stored,current,operator):current;setFormula(`${formula}${display}${next}`);setStored(base);setDisplay(format(base));setOperator(next);setFresh(true)};
  const equals=()=>{if(stored===null||!operator)return;const result=calculate(stored,numberValue(),operator);setFormula(`${formula}${fresh?"":display}`);setDisplay(format(result));setStored(null);setOperator(null);setFresh(true)};
  const clear=()=>{try{localStorage.removeItem("bubbsun-budget-calculator")}catch{}setDisplay("0");setFormula("");setStored(null);setOperator(null);setFresh(true)},shownFormula=formula+(operator&&!fresh?display:"");
  return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="modal budget-calculator"><header><Calculator/><div><small>BUDGETVERKTYG</small><h2>MINIRÄKNARE</h2></div><button className="modal-x" aria-label="Stäng miniräknaren" onClick={onClose}><X/></button></header><output aria-live="polite"><small>{shownFormula||"\u00a0"}</small><strong>{display}</strong></output><div className="budget-calculator-grid"><button className="clear" onClick={clear}>C</button><button onClick={()=>{if(formula&&stored===null&&!operator)setFormula("");setDisplay(value=>value.startsWith("-")?value.slice(1):value==="0"?value:`-${value}`)}}>±</button><button onClick={()=>{if(formula&&stored===null&&!operator)setFormula("");setDisplay(format(numberValue()/100))}}>%</button><button className={operator==="÷"?"active":""} onClick={()=>choose("÷")}>÷</button>{["7","8","9"].map(value=><button key={value} onClick={()=>digit(value)}>{value}</button>)}<button className={operator==="×"?"active":""} onClick={()=>choose("×")}>×</button>{["4","5","6"].map(value=><button key={value} onClick={()=>digit(value)}>{value}</button>)}<button className={operator==="−"?"active":""} onClick={()=>choose("−")}>−</button>{["1","2","3"].map(value=><button key={value} onClick={()=>digit(value)}>{value}</button>)}<button className={operator==="+"?"active":""} onClick={()=>choose("+")}>+</button><button className="zero" onClick={()=>digit("0")}>0</button><button onClick={()=>{if(!fresh&&display.includes(","))return;if(fresh){if(formula&&stored===null&&!operator)setFormula("");setDisplay("0,")}else setDisplay(`${display},`);setFresh(false)} }>,</button><button className="equals" onClick={equals}>=</button></div></section></div>
}
function BudgetSettingsEditor({settings,privateMode,groups,groupBudgetSettings,focusCategoryBudgets=false,onClose,onSave,onReset,onClearMoney}:{settings:BudgetSettings;privateMode:boolean;groups:Record<string,Group>;groupBudgetSettings:Record<string,BudgetSettings>;focusCategoryBudgets?:boolean;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  ...
  const categoryBudgetRef=useRef<HTMLElement|null>(null);
  useEffect(()=>{if(!focusCategoryBudgets)return;const frame=window.requestAnimationFrame(()=>categoryBudgetRef.current?.scrollIntoView({block:"start",behavior:"smooth"}));return()=>window.cancelAnimationFrame(frame)},[focusCategoryBudgets]);
...
function CalendarEventCard({event,lists,members,privateMode,account,onClose,onOpenList,onEdit}:{event:CalendarOccurrence;lists:BubbsunList[];members:Membership[];privateMode:boolean;account:Account;onClose:()=>void;onOpenList:(list:BubbsunList)=>void;onEdit:()=>void}){
  ...
  return <div className="modal-backdrop"><article className={`modal calendar-event-card calendar-event-card-v2${event.category==="birthday"?" birthday":""}`} style={style}>
    ...
    {event.note&&<section className="calendar-card-note"><small>✎ &nbsp; MEMO</small><p>{event.note}</p></section>}
...
function CalendarEditor({event,lists,account,memberships,groups,currentLocation,existing,onClose,onSave,onDelete}:{event:CalendarEvent;lists:BubbsunList[];account:Account;memberships:Membership[];groups:Record<string,Group>;currentLocation:string;existing:boolean;onClose:()=>void;onSave:(event:CalendarEvent,targetLocations:string[],previousLocations:string[])=>Promise<void>;onDelete?:(mode:"single"|"all")=>Promise<void>}){
  ...
  return <div className="modal-backdrop"><div className="modal calendar-editor"><button className="calendar-editor-close" aria-label="Stäng" onClick={onClose}><X/></button><CalendarDays/><h2>{existing?"REDIGERA HÄNDELSE":"NY HÄNDELSE"}</h2>
    ...
    <label>MEMO <small>(valfritt)</small><textarea value={note} maxLength={300} onChange={e=>setNote(e.target.value)} placeholder="Något som är bra att komma ihåg…"/></label>
...
function HelpPage({ onPage }: { onPage: (page: Page) => void }) {
  const guides = [
    ...
    {
      icon: "📒", title: "Skriv ett memo", intro: "Spara text som du vill komma ihåg.",
      steps: ["Öppna menyn och tryck på Memo.", "Välj Privat om bara du ska läsa. Välj en grupp om ni ska läsa tillsammans.", "Tryck på plus, skriv en rubrik och din text.", "Tryck på Spara. Du kan öppna memot igen och ändra det när du vill."],
    },
    {
    ...
    <section className="content subpage">
      ...
      {[
        ...
        [
          ...
          "Administratörens användaröversikt visar endast antal listor, poster, memo, kalenderposter, grupper och följningar samt de senaste 10 besökstidpunkterna – aldrig namn eller innehåll från dina listor och memo.",
        ],
      ].map(([title, body]) => (
...
function VersionsPage({ onPage }: { onPage: (page: Page) => void }) {
  const milestones=[
    ...
    {version:"0.800",eyebrow:"VARDAGEN VÄXER",title:"Fler saker än listor",icon:"📒",tone:"blue",text:"Gränssnittet byggdes om och nya vardagsverktyg fick egna tydliga platser.",points:["Memo och nya specialiserade listor","Snabbare gruppbyte och tydligare medlemsvyer","Stabilare sparning, fler ikoner och bättre mobilstöd"]},
...
function AdminUserStatsDialog({account,lists,counts,onClose}:{account:Account;lists:BubbsunList[];counts?:AdminUserCounts;onClose:()=>void}){
  ...
  const stats=[
    ...
    ["MEMO",counts?.notes||0],["KALENDERPOSTER",counts?.calendarEvents||0],["RECEPT",counts?.recipes||0],["GRUPPER",counts?.groups||0],["FÖLJDA LISTOR",counts?.followedLists||0],["BESÖK",account.visitCount||visits.length],
  ] as const;
...
function NotificationsPage({entries,seenAt,onOpen}:{entries:ActivityEntry[];seenAt:number;onOpen:(entry:ActivityEntry)=>void}) {
  ...
  return <section className="content subpage notifications-page">
    ...
    </button>)}</div>:<div className="notifications-empty"><Bell/><strong>Inget nytt ännu</strong><span>När något händer i dina listor, memo, recept eller kalender syns det här.</span></div>}
  </section>;
}
...
function AuthenticatedApp() {
  ...
  useEffect(()=>{if(!user||!databaseReady||!privateMode||!needsNotes)return;let stopped=false;void loadPrivateNotes(user.uid).then(values=>{if(!stopped)setPrivateNotes(values)}).catch(error=>console.error("Privata memon kunde inte hämtas",error));return()=>{stopped=true}},[user?.uid,databaseReady,privateMode,needsNotes]);
  ...
  useEffect(()=>{
    ...
    for(const note of notes){
      ...
      if(previous&&version>previous&&note.history?.[0]?.uid!==user.uid&&followedNoteIds.has(note.id)&&"Notification" in window&&Notification.permission==="granted")new Notification(`Bubbsun · ${note.title}`,{body:"Memot har ändrats. Tryck för att öppna Bubbsun.",icon:"/assets/bubbsun-header-illustrated.png"});
      notifiedVersions.current[key]=Math.max(previous||0,version);
    }
  ...
  const persistNote=async(note:BubbsunNote,isPrivate=selectedNotePrivate):Promise<boolean>=>{if(!user||!account)return false;const changed=note.title!==selectedNote?.title||note.text!==selectedNote?.text||note.icon!==selectedNote?.icon||note.color!==selectedNote?.color;const entry={uid:user.uid,name:account.displayName,at:Date.now()},complete={...note,creatorId:note.creatorId||user.uid,creatorName:note.creatorName||account.displayName,creatorColor:note.creatorColor??account.personalColor,history:changed?[entry,...(note.history||[])].slice(0,20):note.history||[]};try{if(isPrivate){await savePrivateNote(user.uid,complete);setPrivateNotes(current=>current.map(item=>item.id===complete.id?complete:item))}else if(account.activeGroupId)await saveNote(account.activeGroupId,complete);else return false;setSelectedNote(complete);return true;}catch(error){console.error("Could not save note",error);window.alert("Memot kunde inte sparas. Försök igen.");return false;}};
  const createNote=async(note:BubbsunNote)=>{if(!user||!account)return;const entry={uid:user.uid,name:account.displayName,at:Date.now()},complete={...note,creatorId:user.uid,creatorName:account.displayName,creatorColor:account.personalColor,history:[entry],order:activeNotes.length};try{if(privateMode){await savePrivateNote(user.uid,complete);setPrivateNotes(current=>[...current,complete]);}else if(account.activeGroupId){await saveNote(account.activeGroupId,complete);setNotes(current=>[...current,complete]);}else return;setAddingNote(false);openNote(complete,privateMode);}catch(error){console.error("Could not create note",error);window.alert("Memot kunde inte sparas. Försök igen.");}};
