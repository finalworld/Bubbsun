...
function NotesPage({notes,privateMode,groupName,onMode,onLists,onHelp,onOpen,onReorder}:{notes:BubbsunNote[];privateMode:boolean;groupName:string;onMode:(value:boolean)=>void;onLists:()=>void;onHelp:()=>void;onOpen:(note:BubbsunNote)=>void;onReorder:(from:number,to:number)=>void}){
  const sensors=useSensors(useSensor(MouseSensor,{activationConstraint:{distance:5}}),useSensor(TouchSensor,{activationConstraint:{delay:180,tolerance:8}}));
  ...
  return <section className="content list-page notes-page"><div className="space-tabs space-tabs-refined"><button className={privateMode?"selected":""} onClick={()=>onMode(true)}><LockKeyhole/><span>PRIVAT</span></button><button className={!privateMode?"selected":""} onClick={()=>onMode(false)}><Users/><span>GRUPP<small>{groupName}</small></span></button></div><div className="content-tabs content-tabs-refined"><button onClick={onLists}><ListChecks/> LISTOR</button><button className="selected"><NotebookPen/> ANTECKNINGAR</button></div><div className="notes-heading"><div><h2>{privateMode?"MINA ANTECKNINGAR":"GRUPPENS ANTECKNINGAR"}</h2><p>{privateMode?"Bara du ser dessa.":`I ${groupName}.`}</p></div></div>{!notes.length&&<div className="empty-card"><NotebookPen/><strong>{privateMode?"Här är tomt än så länge":"Gruppen har inga anteckningar än"}</strong><span>{privateMode?"Skapa din första anteckning med plusknappen uppe till höger.":"Skapa gruppens första anteckning med plusknappen uppe till höger."}</span><button type="button" onClick={onHelp}><span>👋</span><span><b>NY HÄR?</b><small>Här kan du skriva idéer, planer och sådant du vill minnas.</small></span><ChevronRight/></button></div>}<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={end}><SortableContext items={notes.map(note=>note.id)} strategy={rectSortingStrategy}><div className="note-stack">{notes.map(note=><SortableNoteCard key={note.id} note={note} onOpen={()=>onOpen(note)}/>)}</div></SortableContext></DndContext></section>
}
function SortableNoteCard({note,onOpen}:{note:BubbsunNote;onOpen:()=>void}){const sortable=useSortable({id:note.id});return <article ref={sortable.setNodeRef} style={{transform:CSS.Transform.toString(sortable.transform),transition:sortable.transition} as CSSProperties} className={`note-card ${sortable.isDragging?"dragging":""}`} onClick={onOpen}><span className="note-color" style={{background:rgbaHex(note.color)}}><img src={noteIconSource(note.icon)} alt=""/></span><div className="note-copy"><strong>{note.title}</strong><small>{note.text.trim()||"Tom anteckning"}</small><em>✍ {note.creatorName||"Bubbsun"}</em></div><button aria-label="Flytta anteckning" onClick={event=>event.stopPropagation()} {...sortable.attributes} {...sortable.listeners}><GripVertical/></button><ChevronRight/></article>}
function NoteEditorPage({note,onSave,onBack,onDelete}:{note:BubbsunNote;onSave:(note:BubbsunNote)=>void;onBack:()=>void;onDelete:()=>void}){const [title,setTitle]=useState(note.title),[text,setText]=useState(note.text),[logOpen,setLogOpen]=useState(false);useEffect(()=>{setTitle(note.title);setText(note.text)},[note.id,note.title,note.text]);return <section className="content note-editor-page"><button className="back-button" onClick={onBack}>‹ ANTECKNINGAR</button><article><div className="note-editor-title"><span style={{background:rgbaHex(note.color)}}><img src={noteIconSource(note.icon)} alt=""/></span><input value={title} onChange={event=>setTitle(event.target.value)} maxLength={80} placeholder="Rubrik"/><button className="note-log-button" onClick={()=>setLogOpen(true)} title="Visa ändringslogg">🕘</button></div><p className="note-created">Skapad av {note.creatorName||"Bubbsun"}</p><textarea value={text} onChange={event=>setText(event.target.value)} placeholder="Skriv din anteckning här…"/><div className="note-editor-actions"><button className="danger" onClick={()=>{if(window.confirm("Ta bort anteckningen?"))onDelete()}}><Trash2/> TA BORT</button><span/><button className="cancel" onClick={onBack}>AVBRYT</button><button onClick={()=>{if(title.trim())onSave({...note,title:title.trim(),text})}}>SPARA</button></div></article>{logOpen&&<div className="modal-backdrop"><div className="modal note-log-modal"><button className="modal-close" onClick={()=>setLogOpen(false)}><X/></button><h2>ÄNDRINGSLOGG</h2>{note.history?.length?<ul>{note.history.map((entry,index)=><li key={`${entry.at}-${index}`}><strong>{entry.name}</strong><small>{new Date(entry.at).toLocaleString("sv-SE")}</small></li>)}</ul>:<p>Ingen har sparat några ändringar ännu.</p>}</div></div>}</section>}
function NewNoteEditor({onCancel,onSave}:{onCancel:()=>void;onSave:(note:BubbsunNote)=>void}){const [title,setTitle]=useState(""),[icon,setIcon]=useState("idea"),[color,setColor]=useState(listColorOptions[0]);return <div className="modal-backdrop"><form className="modal note-new-editor" onSubmit={event=>{event.preventDefault();if(title.trim())onSave({id:crypto.randomUUID(),title:title.trim(),text:"",icon,color,order:Date.now(),creatorId:""})}}><h2>NY ANTECKNING</h2><input autoFocus value={title} onChange={event=>setTitle(event.target.value)} placeholder="Vad handlar den om?"/><div className="note-icon-picker">{noteIcons.map(value=><button type="button" key={value} className={icon===value?"selected":""} onClick={()=>setIcon(value)}><img src={noteIconSource(value)} alt=""/></button>)}</div><div className="color-picker">{listColorOptions.map(value=><button type="button" key={value} className={color===value?"selected":""} onClick={()=>setColor(value)} style={{"--choice-color":rgbaHex(value)} as CSSProperties}/>)}</div><div><button type="button" className="cancel" onClick={onCancel}>AVBRYT</button><button type="submit">SKAPA</button></div></form></div>}
...
function AuthenticatedApp() {
  ...
  const persistNote=async(note:BubbsunNote,isPrivate=selectedNotePrivate)=>{if(!user||!account)return;const changed=note.title!==selectedNote?.title||note.text!==selectedNote?.text;const entry={uid:user.uid,name:account.displayName,at:Date.now()},complete={...note,creatorId:note.creatorId||user.uid,creatorName:note.creatorName||account.displayName,history:changed?[entry,...(note.history||[])].slice(0,20):note.history||[]};try{if(isPrivate)await savePrivateNote(user.uid,complete);else if(account.activeGroupId)await saveNote(account.activeGroupId,complete);setSelectedNote(complete);}catch(error){console.error("Could not save note",error);window.alert("Anteckningen kunde inte sparas. Försök igen.");}};
  const createNote=async(note:BubbsunNote)=>{if(!user||!account)return;const entry={uid:user.uid,name:account.displayName,at:Date.now()},complete={...note,creatorId:user.uid,creatorName:account.displayName,history:[entry],order:activeNotes.length};try{if(privateMode)await savePrivateNote(user.uid,complete);else if(account.activeGroupId)await saveNote(account.activeGroupId,complete);setAddingNote(false);openNote(complete,privateMode);}catch(error){console.error("Could not create note",error);window.alert("Anteckningen kunde inte sparas. Försök igen.");}};
  ...
  return (
    <main className={`app-shell theme-${activeTheme.id}`} style={themeStyle}>
      ...
      {page === "notes" && <NotesPage notes={activeNotes} privateMode={privateMode} groupName={groupName} onMode={value=>{setPrivateMode(value);setSelectedNote(null)}} onLists={()=>navigate("lists")} onHelp={()=>navigate("help")} onOpen={note=>openNote(note)} onReorder={(from,to)=>void reorderNotes(from,to)} />}
      {page === "note" && selectedNote && <NoteEditorPage note={selectedNote} onBack={()=>navigate("notes")} onSave={note=>void persistNote(note)} onDelete={async()=>{if(selectedNotePrivate)await removePrivateNote(user.uid,selectedNote.id);else if(account.activeGroupId)await removeNote(account.activeGroupId,selectedNote.id);navigate("notes")}}/>}
      {page === "people" && (
        <PeoplePage
          account={account}
          group={activeGroup}
