...
function RecipePrintButton(){
  ...
  return <><button className="recipe-print-button" type="button" onClick={openPrintChoice}><Printer/> SKRIV UT</button>{open&&<div className="modal-backdrop recipe-print-choice-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}><div className="modal recipe-print-choice-modal"><button className="modal-close recipe-print-choice-close" type="button" aria-label="Stäng" onClick={()=>setOpen(false)}><X/></button><Printer className="recipe-print-choice-icon"/><h2>SKRIV UT RECEPT</h2><p>Vill du ha med den stora receptbilden?</p><div className="recipe-print-choice-actions"><button type="button" onClick={()=>printRecipe(true)}><ImagePlus/><span><b>MED BILD</b><small>Receptbilden följer med</small></span></button><button type="button" onClick={()=>printRecipe(false)}><BookOpen/><span><b>UTAN BILD</b><small>Bara text och små ikoner</small></span></button></div></div></div>}</>
}
...
function RecipesPage({recipes,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onAddToList,onCreateIngredientList,openRecipeId,onRecipeOpened}:{recipes:Recipe[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>Promise<void>;onSave:(recipe:Recipe,targetLocation?:string)=>Promise<void>;onDelete:(recipe:Recipe)=>Promise<void>;onAddToList:(recipe:Recipe,listId:string)=>Promise<void>;onCreateIngredientList:(recipe:Recipe,targetLocation:string)=>Promise<void>;openRecipeId?:string;onRecipeOpened?:()=>void}){
...
function DiscoverRecipesPage({recipes,uid,memberships,groups,onCreateIngredientList}:{recipes:Recipe[];uid:string;memberships:Membership[];groups:Record<string,Group>;onCreateIngredientList:(recipe:Recipe,targetLocation:string)=>Promise<void>}){
  ...
  const pagedRecipes=shown.slice((recipePage-1)*recipesPerPage,recipePage*recipesPerPage);useEffect(()=>setRecipePage(1),[search,category,subcategory,sortMode]);
...
function usePublicRecipeMetadata(recipe:Recipe|null|undefined){
  useEffect(()=>{
    ...
    const title=recipe?`${recipe.title} – recept | Bubbsun`:"Receptet hittades inte | Bubbsun",description=recipe?`${recipeCategoryLabel(recipe)} av ${recipe.creatorName}. ${recipeYieldLabel(recipe)}${recipe.minutes?`, ${recipe.minutes} minuter`:""}.`:"Det publika receptet finns inte längre.";
...
function PublicRecipeArticle({recipe}:{recipe:Recipe}){return <article className="recipe-view public-recipe-view" style={{"--recipe-creator":"var(--theme-accent)"} as CSSProperties}>{recipe.image?<img className="recipe-hero" src={recipe.image} alt={recipe.title}/>:<div className="recipe-hero fallback">🍲</div>}<small>{recipeCategoryLabel(recipe)}</small><h1>{recipe.title}</h1><div className="recipe-view-tools"><RecipePrintButton/><RecipeShareControl recipe={recipe}/></div><div className="recipe-facts"><span>🍽️ {recipeYieldLabel(recipe)}</span>{recipe.minutes>0&&<span>⏱️ {recipe.minutes} minuter</span>}</div><div className="recipe-creator-line">Skapad av {recipe.creatorName}</div>{recipe.description&&<section className="recipe-description"><h3>OM RECEPTET</h3><p>{recipe.description}</p></section>}<section><h3>INGREDIENSER</h3><ul>{recipe.ingredients.map(item=><li key={item.id}><b>{[item.amount,item.unit].filter(Boolean).join(" ")}</b><span>{item.name}</span></li>)}</ul></section><section><h3>GÖR SÅ HÄR</h3><div className="recipe-instructions">{recipeInstructionSteps(recipe.instructions).map((step,index)=><p key={index}><b>{index+1}</b><span>{step}</span></p>)}</div></section>{recipe.note&&<aside><b>ANTECKNING</b><p>{recipe.note}</p></aside>}<footer><a href="/recept">← Upptäck fler recept</a></footer></article>}
