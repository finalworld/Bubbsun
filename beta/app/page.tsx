...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>}){
  ...
  const monthDate=new Date(now.getFullYear(),now.getMonth()+monthOffset,1),monthKey=`${monthDate.getFullYear()}-${String(monthDate.getMonth()+1).padStart(2,"0")}`,monthNumber=monthDate.getFullYear()*12+monthDate.getMonth(),monthEntries=entries.flatMap(entry=>{const start=new Date(`${entry.date}T12:00:00`),startNumber=start.getFullYear()*12+start.getMonth();if(entry.recurrence==="monthly"&&monthNumber>=startNumber){const day=Math.min(start.getDate(),new Date(monthDate.getFullYear(),monthDate.getMonth()+1,0).getDate());return[{entry,occurrenceDate:`${monthKey}-${String(day).padStart(2,"0")}`}]}return entry.date.startsWith(monthKey)?[{entry,occurrenceDate:entry.date}]:[]}).sort((a,b)=>new Date(`${b.occurrenceDate}T12:00:00`).getTime()-new Date(`${a.occurrenceDate}T12:00:00`).getTime()||(b.entry.createdAt||0)-(a.entry.createdAt||0)),income=monthEntries.filter(item=>item.entry.type==="income").reduce((sum,item)=>sum+item.entry.amount,0),expense=monthEntries.filter(item=>item.entry.type==="expense").reduce((sum,item)=>sum+item.entry.amount,0),activeGroup=groups[account.activeGroupId];
  ...
  return <section className="content budget-page">
    ...
    <BudgetAccountOverview settings={settings} entries={monthEntries.map(item=>item.entry)} onSelect={setSelectedAccountId}/><section className="budget-list"><h2>MÅNADENS POSTER</h2>{monthEntries.length?monthEntries.map(({entry,occurrenceDate})=><button key={entry.id} className="budget-row" onClick={()=>{if(entry.type==="transfer"){setEditingTransfer(entry);setTransferOpen(true)}else setEditing(entry)}}><span className={entry.type}>{entry.type==="income"?"+":entry.type==="expense"?"−":"⇄"}</span><span><strong>{entry.title}</strong><small>{entry.type==="transfer"?`${accountName(entry.fromAccountId)} → ${accountName(entry.toAccountId)}`:<>{entry.category}{entry.subcategory?` · ${entry.subcategory}`:""} · {accountName(entry.accountId)}</>} · {new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short"}).format(new Date(`${occurrenceDate}T12:00:00`))}</small></span><b className={entry.type}>{entry.type==="income"?"+ ":entry.type==="expense"?"− ":""}{budgetMoney(entry.amount)}</b><ChevronRight/></button>):<div className="budget-empty"><WalletCards/><h3>Inga poster</h3></div>}</section></>:<section className="budget-list budget-recurring-list"><header><div><small>AUTOMATISKA POSTER</small><h2>ÅTERKOMMANDE</h2></div><b>{entries.filter(entry=>entry.recurrence==="monthly").length} st</b></header>{entries.filter(entry=>entry.recurrence==="monthly").sort((a,b)=>a.date.localeCompare(b.date)).map(entry=><button key={entry.id} className="budget-row" onClick={()=>{if(entry.type==="transfer"){setEditingTransfer(entry);setTransferOpen(true)}else setEditing(entry)}}><span className={entry.type}>{entry.type==="income"?"+":entry.type==="expense"?"−":"⇄"}</span><span><strong>{entry.title}</strong><small>{entry.type==="transfer"?"Överföring":entry.category} · varje månad den {Number(entry.date.slice(-2))}:e</small></span><b className={entry.type}>{entry.type==="income"?"+ ":entry.type==="expense"?"− ":""}{budgetMoney(entry.amount)}</b><ChevronRight/></button>)}{!entries.some(entry=>entry.recurrence==="monthly")&&<div className="budget-empty"><History/><h3>Inga återkommande poster</h3><p>Markera “Återkommer varje månad” när du skapar en post.</p></div>}</section>}
    {selectedAccountId&&<BudgetAccountDetail accountId={selectedAccountId} settings={settings} entries={monthEntries} onClose={()=>setSelectedAccountId(undefined)}/>}
...
function BudgetAccountOverview({settings,entries,onSelect}:{settings:BudgetSettings;entries:BudgetEntry[];onSelect:(id:string)=>void}){if(!settings.banks.length)return null;const accountTotal=(id:string)=>entries.reduce((sum,entry)=>entry.type==="transfer"?sum+(entry.toAccountId===id?entry.amount:0)-(entry.fromAccountId===id?entry.amount:0):entry.accountId===id?sum+(entry.type==="income"?entry.amount:-entry.amount):sum,0);return <section className="budget-accounts-overview"><header><span><small>DIN MÅNADSÖVERSIKT</small><h2>BANKER & KONTON</h2></span></header><div>{settings.banks.map(bank=><article key={bank.id}><header><span>▣</span><strong>{bank.name}</strong><b>{budgetMoney(bank.accounts.reduce((sum,item)=>sum+accountTotal(item.id),0))}</b></header><div>{bank.accounts.map(account=>{const total=accountTotal(account.id);return <button type="button" key={account.id} onClick={()=>onSelect(account.id)}><span><i>{budgetAccountIcon(account.icon)}</i><strong>{account.name}</strong></span><b className={total<0?"expense":"income"}>{budgetMoney(total)}</b><ChevronRight/></button>})}</div></article>)}</div></section>}
function BudgetAccountDetail({accountId,settings,entries,onClose}:{accountId:string;settings:BudgetSettings;entries:Array<{entry:BudgetEntry;occurrenceDate:string}>;onClose:()=>void}){const match=settings.banks.flatMap(bank=>bank.accounts.map(account=>({bank:bank.name,account}))).find(item=>item.account.id===accountId);if(!match)return null;const relevant=entries.filter(({entry})=>entry.type==="transfer"?entry.fromAccountId===accountId||entry.toAccountId===accountId:entry.accountId===accountId),signed=(entry:BudgetEntry)=>entry.type==="transfer"?(entry.toAccountId===accountId?entry.amount:-entry.amount):entry.type==="income"?entry.amount:-entry.amount,total=relevant.reduce((sum,{entry})=>sum+signed(entry),0),accountLabel=(id?:string)=>settings.banks.flatMap(bank=>bank.accounts.map(account=>({id:account.id,label:`${bank.name} · ${account.name}`}))).find(item=>item.id===id)?.label||(id===budgetUnassignedAccountId?"Ej placerat":"Okänt konto");return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="modal budget-account-detail"><button className="modal-x" onClick={onClose}><X/></button><header><i>{budgetAccountIcon(match.account.icon)}</i><span><small>{match.bank}</small><h2>{match.account.name}</h2></span><b className={total<0?"expense":"income"}>{budgetMoney(total)}</b></header><div className="budget-account-detail-summary"><span>BERÄKNAT KONTOBELOPP</span><strong>{relevant.length} {relevant.length===1?"händelse":"händelser"}</strong></div><div className="budget-account-detail-list">{relevant.map(({entry,occurrenceDate})=>{const value=signed(entry);return <article key={`${entry.id}-${occurrenceDate}`}><i className={value<0?"expense":"income"}>{entry.type==="transfer"?"⇄":value>0?"+":"−"}</i><span><strong>{entry.title}</strong><small>{entry.type==="transfer"?`${accountLabel(entry.fromAccountId)} → ${accountLabel(entry.toAccountId)}`:entry.category} · {new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short"}).format(new Date(`${occurrenceDate}T12:00:00`))}</small></span><b className={value<0?"expense":"income"}>{value>0?"+ ":"− "}{budgetMoney(Math.abs(value))}</b></article>})}{!relevant.length&&<div className="budget-empty"><WalletCards/><h3>Inga händelser denna månad</h3></div>}</div><p className="budget-account-detail-note">Beloppet räknas av posterna som är kopplade till kontot. Att välja konto på en befintlig post flyttar posten från “Ej placerat” – den skapar inte en ny inkomst eller utgift.</p></section></div>}
...
function BudgetSettingsEditor({settings,onClose,onSave}:{settings:BudgetSettings;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>}){const [banks,setBanks]=useState(settings.banks);return <div className="modal-backdrop"><section className="modal budget-settings-editor"><button className="modal-x" onClick={onClose}><X/></button><header><Settings/><h2>BUDGETINSTÄLLNINGAR</h2></header><div className="budget-bank-editor">{banks.map(bank=><section key={bank.id}><div><input value={bank.name} placeholder="Namn på bank" onChange={event=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,name:event.target.value}:item))}/><button onClick={()=>setBanks(old=>old.filter(item=>item.id!==bank.id))}><Trash2/></button></div>{bank.accounts.map(account=><div className="budget-account-edit" key={account.id}><select aria-label="Kontoikon" title="Välj kontoikon" value={account.icon||"wallet"} onChange={event=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,accounts:item.accounts.map(value=>value.id===account.id?{...value,icon:event.target.value}:value)}:item))}>{budgetAccountIcons.map(item=><option value={item.id} key={item.id}>{item.icon} {item.label}</option>)}</select><input value={account.name} placeholder="Namn på konto" onChange={event=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,accounts:item.accounts.map(value=>value.id===account.id?{...value,name:event.target.value}:value)}:item))}/><button onClick={()=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,accounts:item.accounts.filter(value=>value.id!==account.id)}:item))}><Trash2/></button></div>)}<button className="add-account" onClick={()=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,accounts:[...item.accounts,{id:crypto.randomUUID(),name:"",icon:"wallet"}]}:item))}><Plus/> LÄGG TILL KONTO</button></section>)}</div><button className="add-bank" onClick={()=>setBanks(old=>[...old,{id:crypto.randomUUID(),name:"",accounts:[]}])}><Plus/> LÄGG TILL BANK</button><footer><button onClick={onClose}>AVBRYT</button><button onClick={()=>onSave({banks,updatedAt:Date.now()})}>SPARA</button></footer></section></div>}
...
function AuthenticatedApp() {
  ...
  const [budgetSettings,setBudgetSettings]=useState<BudgetSettings>({banks:[],updatedAt:0});
  const [privateBudgetSettings,setPrivateBudgetSettings]=useState<BudgetSettings>({banks:[],updatedAt:0});
  ...
  useEffect(()=>{if(!account?.activeGroupId||privateMode){setBudgetSettings({banks:[],updatedAt:0});return;}return watchBudgetSettings(account.activeGroupId,setBudgetSettings);},[account?.activeGroupId,privateMode]);
  ...
  const persistBudgetEntry=async(entry:BudgetEntry)=>{if(!user||!account)return;const complete={...entry,accountId:entry.accountId||activeBudgetSettings.banks.flatMap(bank=>bank.accounts)[0]?.id||""};if(privateMode)await savePrivateBudgetEntry(user.uid,complete);else if(account.activeGroupId)await saveBudgetEntry(account.activeGroupId,complete)};
