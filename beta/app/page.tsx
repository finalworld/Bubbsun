...
function BudgetCalculator({onClose}:{onClose:()=>void}){
  const [display,setDisplay]=useState("0"),[formula,setFormula]=useState(""),[stored,setStored]=useState<number|null>(null),[operator,setOperator]=useState<"+"|"−"|"×"|"÷"|null>(null),[fresh,setFresh]=useState(true);
  ...
  const clear=()=>{setDisplay("0");setFormula("");setStored(null);setOperator(null);setFresh(true)},shownFormula=formula+(operator&&!fresh?display:"");
