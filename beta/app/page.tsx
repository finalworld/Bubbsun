...
function NoteEditorPage({note,onSave,onBack,onDelete,follow,creator}:{note:BubbsunNote;onSave:(note:BubbsunNote)=>Promise<boolean>;onBack:()=>void;onDelete:()=>void;follow?:{uid:string;groupId:string};creator?:{name:string;color:number}}){
...
function ListPage({
  ...
  return (
    <section className="content detail-page">
      ...
      <div className={`add-panel ${list.listType === "wishlist" ? "add-panel-wishlist" : list.listType === "packing" ? "add-panel-packing" : ""}`}>
        <h2>LÄGG TILL</h2>
        <div>
