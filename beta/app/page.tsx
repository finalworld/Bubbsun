...
function LinkImageCropModal({source,onCancel,onApply}:{source:string;onCancel:()=>void;onApply:(image:string)=>void}){const [zoom,setZoom]=useState(1),[x,setX]=useState(0),[y,setY]=useState(0),[busy,setBusy]=useState(false),drag=useRef<{x:number;y:number;valueX:number;valueY:number;pointerId:number}|null>(null);const clamp=(value:number)=>Math.max(-100,Math.min(100,value));return <div className="modal-backdrop link-crop-backdrop"><div className="link-crop-modal"><h2>JUSTERA BILDEN</h2><div className="link-crop-viewport" onPointerDown={event=>{drag.current={x:event.clientX,y:event.clientY,valueX:x,valueY:y,pointerId:event.pointerId};event.currentTarget.setPointerCapture(event.pointerId)}} onPointerMove={event=>{const start=drag.current;if(!start||start.pointerId!==event.pointerId)return;const box=event.currentTarget.getBoundingClientRect();setX(clamp(start.valueX-((event.clientX-start.x)/box.width)*140));setY(clamp(start.valueY-((event.clientY-start.y)/box.height)*140))}} onPointerUp={event=>{if(drag.current?.pointerId===event.pointerId)drag.current=null}} onPointerCancel={()=>{drag.current=null}}><img src={source} alt="Bildutsnitt" style={{transform:`translate(${-x*.32}%,${-y*.32}%) scale(${zoom})`}}/></div><p className="link-crop-hint">Ta tag i bilden och dra den till rätt läge.</p><label>ZOOM<input type="range" min="1" max="3" step="0.05" value={zoom} onChange={event=>setZoom(Number(event.target.value))}/></label><div><button className="cancel" disabled={busy} onClick={onCancel}>AVBRYT</button><button disabled={busy} onClick={async()=>{setBusy(true);try{onApply(await cropLinkImage(source,zoom,x,y))}finally{setBusy(false)}}}>{busy?"SPARAR…":"ANVÄND UTSNITT"}</button></div></div></div>}
