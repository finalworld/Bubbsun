...
const expandCalendarEvents=(events:CalendarEvent[],from:string,to:string):CalendarOccurrence[]=>events.flatMap(event=>{const days=event.recurrenceDays||[];if(!days.length)return event.date>=from&&event.date<=to?[{...event,occurrenceDate:event.date}]:[];const start=new Date(`${event.date}T12:00:00`),fromDate=new Date(`${from}T12:00:00`);if(start<fromDate)start.setTime(fromDate.getTime());const until=event.recurrenceForever||!event.recurrenceUntil?to:event.recurrenceUntil<to?event.recurrenceUntil:to,values:CalendarOccurrence[]=[];for(const cursor=new Date(start);calendarDateKey(cursor)<=until;cursor.setDate(cursor.getDate()+1)){const key=calendarDateKey(cursor);if(days.includes(cursor.getDay()))values.push({...event,occurrenceDate:key})}return values});
...
function CalendarPage({events,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onOpenList}:{events:CalendarEvent[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete:(event:CalendarEvent)=>Promise<void>;onOpenList:(list:BubbsunList)=>void}){
  ...
  return <section className="content calendar-page">
    ...
    {range==="7"&&<div className="calendar-week-nav"><button aria-label="Föregående vecka" onClick={()=>setWeekOffset(value=>value-1)}><ChevronLeft/></button><strong>Vecka {weekNumber} – {weekMonths}</strong><button aria-label="Nästa vecka" onClick={()=>setWeekOffset(value=>value+1)}><ChevronRight/></button></div>}
    {range==="7"?<div className="calendar-week">{weekDates.map(date=>{const day=new Date(`${date}T12:00:00`).getDay();return <section key={date} className={`${date===today?"today ":""}${day===0||day===6?"weekend":""}`.trim()}><h2><strong>{new Intl.DateTimeFormat("sv-SE",{weekday:"short"}).format(new Date(`${date}T12:00:00`))}</strong><b>{Number(date.slice(-2))}</b></h2><div onClick={()=>{setCreateDate(date);onCreating(true)}}>{(grouped[date]||[]).map(event=>eventCard(event,true))}</div></section>})}</div>:!visible.length?<div className="calendar-empty"><CalendarDays/><strong>Inget planerat här</strong><span>Skönt — eller dags att lägga till något?</span><button onClick={()=>{setCreateDate(today);onCreating(true)}}><Plus/> LÄGG TILL</button></div>:<div className="calendar-agenda">{Object.entries(grouped).map(([date,items])=><section key={date}><h2>{calendarDayLabel(date)}</h2>{items.map(event=>eventCard(event))}</section>)}{range==="agenda"&&agendaLimit<visible.length&&<div ref={agendaLoader} className="calendar-loader">LADDAR FLER…</div>}</div>}
...
function CalendarEventCard({event,lists,members,privateMode,account,onClose,onOpenList,onEdit}:{event:CalendarOccurrence;lists:BubbsunList[];members:Membership[];privateMode:boolean;account:Account;onClose:()=>void;onOpenList:(list:BubbsunList)=>void;onEdit:()=>void}){
  const category=calendarCategory(event.category),creator=privateMode?null:members.find(member=>member.uid===event.creatorId),creatorName=privateMode?account.displayName:(creator?.displayName||event.creatorName),linked=(event.linkedListIds||[]).map(id=>lists.find(list=>list.id===id)).filter((list):list is BubbsunList=>Boolean(list)),dateLabel=new Intl.DateTimeFormat("sv-SE",{weekday:"long",day:"numeric",month:"long",year:"numeric"}).format(new Date(`${event.occurrenceDate}T12:00:00`)),timeLabel=event.allDay||!event.time?"Hela dagen":event.endTime?`${event.time}–${event.endTime}`:event.time,reminderLabel=event.reminderMinutes===15?"15 minuter innan":event.reminderMinutes===60?"1 timme innan":event.reminderMinutes===720?"12 timmar innan":event.reminderMinutes===1440?"24 timmar innan":"Ingen påminnelse",repeat=Boolean(event.recurrenceDays?.length),style={"--event-color":rgbaHex(event.color||account.personalColor||colorOptions[0]),"--creator-color":rgbaHex(privateMode?(account.personalColor||colorOptions[0]):(creator?.color||account.personalColor||colorOptions[0]))} as CSSProperties;
  return <div className="modal-backdrop"><article className={`modal calendar-event-card${event.category==="birthday"?" birthday":""}`} style={style}><button className="calendar-card-close" aria-label="Stäng" onClick={onClose}><X/></button><header className="calendar-card-hero"><div className="calendar-card-icon">{category.icon||"📅"}</div><div><small>{category.label||"Kalenderpost"}</small><h2>{event.title}</h2></div></header><div className="calendar-card-facts"><p><CalendarDays/><span><small>DATUM</small><strong>{dateLabel}</strong></span></p><p><Bell/><span><small>TID & PÅMINNELSE</small><strong>{timeLabel} · {reminderLabel}</strong></span></p>{repeat&&<p><History/><span><small>ÅTERKOMMANDE</small><strong>{event.recurrenceForever?"Återkommer för alltid":event.recurrenceUntil?`Till ${event.recurrenceUntil}`:"Ja"}</strong></span></p>}</div>{event.note&&<section className="calendar-card-note"><small>ANTECKNING</small><p>{event.note}</p></section>}{linked.length>0&&<section className="calendar-card-lists"><small>KOPPLADE LISTOR</small>{linked.map(list=><button key={list.id} onClick={()=>onOpenList(list)}><span>🔗</span><strong>{list.name}</strong><ChevronRight/></button>)}</section>}<footer className="calendar-card-footer"><p className="calendar-card-creator" style={{borderColor:"var(--creator-color)"}}>Skapad av {creatorName}</p><button className="calendar-card-edit" onClick={onEdit}><Pencil/> REDIGERA</button></footer></article></div>
}

function CalendarEditor({event,lists,existing,onClose,onSave,onDelete}:{event:CalendarEvent;lists:BubbsunList[];existing:boolean;onClose:()=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete?:()=>Promise<void>}){
  const [title,setTitle]=useState(event.title),[date,setDate]=useState(event.date),[time,setTime]=useState(event.time||""),[endTime,setEndTime]=useState(event.endTime||""),[allDay,setAllDay]=useState(event.allDay===true),[category,setCategory]=useState(event.category??""),[color,setColor]=useState(event.color||colorOptions[0]),[repeat,setRepeat]=useState(Boolean(event.recurrenceDays?.length)),[repeatDays,setRepeatDays]=useState<number[]>(event.recurrenceDays||[]),[forever,setForever]=useState(event.recurrenceDays?.length?event.recurrenceForever!==false:true),[until,setUntil]=useState(event.recurrenceUntil||""),[note,setNote]=useState(event.note||""),[linkedListIds,setLinkedListIds]=useState<string[]>((event.linkedListIds||[]).slice(0,3)),[reminderMinutes,setReminderMinutes]=useState(event.reminderMinutes||0),[busy,setBusy]=useState(false);
...
function AuthenticatedApp() {
  ...
  const persistCalendarEvent=async(event:CalendarEvent)=>{if(!user||!account)return;const complete:CalendarEvent={id:event.id,title:event.title,date:event.date,time:event.time,endTime:event.endTime,allDay:event.allDay,category:event.category,color:event.color||account.personalColor||colorOptions[0],recurrenceDays:event.recurrenceDays,recurrenceForever:event.recurrenceForever,recurrenceUntil:event.recurrenceUntil,note:event.note,linkedListIds:(event.linkedListIds||[]).slice(0,3),reminderMinutes:event.reminderMinutes||0,creatorId:event.creatorId||user.uid,creatorName:event.creatorName||account.displayName,createdAt:event.createdAt||Date.now(),updatedAt:Date.now()};if(privateMode)await savePrivateCalendarEvent(user.uid,complete);else if(account.activeGroupId)await saveCalendarEvent(account.activeGroupId,complete);};
  const deleteCalendarEvent=async(event:CalendarEvent)=>{if(!user||!account)return;if(!window.confirm(`Ta bort ”${event.title}”?`))return;if(privateMode)await removePrivateCalendarEvent(user.uid,event.id);else if(account.activeGroupId)await removeCalendarEvent(account.activeGroupId,event.id);};
