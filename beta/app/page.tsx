...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset,onClearMoney}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  ...
  return <section className="content budget-page">
    ...
    <section className="budget-insights"><article><small>JÄMFÖRT MED FÖRRA MÅNADEN</small><strong className={expense>previousExpense?"expense":"income"}>{expense===previousExpense?"Samma utgifter":`${budgetMoney(Math.abs(expense-previousExpense))} ${expense>previousExpense?"mer":"mindre"}`}</strong></article><article><small>KOMMANDE DENNA MÅNAD</small><strong>{upcomingExpenses.length} {upcomingExpenses.length===1?"utgift":"utgifter"} · {budgetMoney(upcomingExpenses.reduce((sum,item)=>sum+item.entry.amount,0))}</strong><span>{upcomingIncomes.length} {upcomingIncomes.length===1?"inkomst":"inkomster"} · + {budgetMoney(upcomingIncomes.reduce((sum,item)=>sum+item.entry.amount,0))}{upcomingTransfers.length?` · ${upcomingTransfers.length} ${upcomingTransfers.length===1?"överföring":"överföringar"}`:""}</span></article></section>
