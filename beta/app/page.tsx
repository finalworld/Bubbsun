      ...
      <aside className={`drawer ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="drawer-footer">
          ...
          <small className="drawer-version-text">
            Bubbsun v0.849 · Web Edition Beta
          </small>
        </div>
      </aside>
    </>
...
function CalendarPage({events,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onOpenList}:{events:CalendarEvent[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete:(event:CalendarEvent)=>Promise<void>;onOpenList:(list:BubbsunList)=>void}){
  ...
  const activeGroup=groups[account.activeGroupId];
  const changeWeek=async(direction:-1|1)=>{if(weekAnimating.current)return;const element=calendarWeekRef.current,reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(!element||reduced){setWeekOffset(value=>value+direction);return}weekAnimating.current=true;await element.animate([{transform:"translateX(0)",opacity:1},{transform:`translateX(${direction<0?"18px":"-18px"})`,opacity:.15}],{duration:145,easing:"ease-in",fill:"forwards"}).finished;setWeekOffset(value=>value+direction);await new Promise<void>(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve())));await element.animate([{transform:`translateX(${direction<0?"-18px":"18px"})`,opacity:.15},{transform:"translateX(0)",opacity:1}],{duration:190,easing:"cubic-bezier(.2,.75,.25,1)",fill:"forwards"}).finished;element.style.transform="";element.style.opacity="";weekAnimating.current=false};
  const changeMonth=async(direction:-1|1)=>{if(monthAnimating.current)return;const element=calendarMonthRef.current,reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;monthAnimating.current=true;if(element&&!reduced)await element.animate([{transform:"translateX(0)",opacity:1},{transform:`translateX(${direction<0?"20px":"-20px"})`,opacity:.12}],{duration:145,easing:"ease-in",fill:"forwards"}).finished;const next=new Date(monthAnchor);next.setMonth(next.getMonth()+direction);setSelectedMonthDate(calendarDateKey(next));setMonthOffset(value=>value+direction);await new Promise<void>(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve())));if(element&&!reduced){await element.animate([{transform:`translateX(${direction<0?"-20px":"20px"})`,opacity:.12},{transform:"translateX(0)",opacity:1}],{duration:195,easing:"cubic-bezier(.2,.75,.25,1)",fill:"forwards"}).finished;element.style.transform="";element.style.opacity=""}monthAnimating.current=false};
  ...
  const eventCard=(event:CalendarOccurrence,compact=false)=>{const creator=privateMode?null:members.find(member=>member.uid===event.creatorId),color=rgbaHex(event.color||creator?.color||account.personalColor||colorOptions[0]),creatorColor=rgbaHex(privateMode?(account.personalColor||colorOptions[0]):(creator?.color||account.personalColor||colorOptions[0])),category=calendarCategory(event.category),birthday=event.category==="birthday",timeLabel=event.allDay||!event.time?"HELA DAGEN":event.endTime?`${event.time}–${event.endTime}`:event.time,style={"--event-color":color,"--creator-color":creatorColor} as CSSProperties,key=`${event.id}-${event.occurrenceDate}`,linked=(event.linkedListIds||[]).map(id=>lists.find(list=>list.id===id)).filter((list):list is BubbsunList=>Boolean(list)),open=(click:{stopPropagation:()=>void})=>{click.stopPropagation();if(weekSwipeHandled.current)return;setViewing(event)};if(compact)return <button className={`calendar-event compact${birthday?" birthday":""}`} key={key} style={style} onClick={open}><span className="calendar-compact-meta">{category.icon&&<b>{category.icon}</b>}<time>{timeLabel}</time>{linked.length>0&&<small>🔗{linked.length}</small>}{Boolean(event.recurrenceDays?.length)&&<small className="calendar-repeat">↻</small>}</span><strong className="calendar-compact-title">{event.title}</strong></button>;return <div role="button" tabIndex={0} className={`calendar-event${birthday?" birthday":""}`} key={key} style={style} onClick={open} onKeyDown={e=>{if(e.key==="Enter")open(e)}}><i/><time>{timeLabel}</time><span><strong>{category.icon&&<b>{category.icon}</b>}{event.title}{Boolean(event.recurrenceDays?.length)&&<small className="calendar-repeat">↻</small>}</strong>{event.note&&<small>{event.note}</small>}{linked.length>0&&<span className="calendar-linked-lists">{linked.map(list=><button key={list.id} onClick={click=>{click.stopPropagation();onOpenList(list)}}>🔗 {list.name}</button>)}</span>}</span><em aria-hidden="true"/><ChevronRight/></div>};
  ...
  return <section className="content calendar-page">
    ...
    {range==="7"?<div ref={calendarWeekRef} className="calendar-week" onTouchStart={touch=>{if(window.innerWidth<=700){const point=touch.touches[0];weekSwipeStart.current=point?{x:point.clientX,y:point.clientY}:null}}} onTouchEnd={touch=>{const start=weekSwipeStart.current,point=touch.changedTouches[0];weekSwipeStart.current=null;if(!start||!point||window.innerWidth>700)return;const dx=point.clientX-start.x,dy=point.clientY-start.y;if(Math.abs(dx)<55||Math.abs(dx)<Math.abs(dy)*1.25)return;weekSwipeHandled.current=true;void changeWeek(dx<0?1:-1);window.setTimeout(()=>{weekSwipeHandled.current=false},450)}}>{weekDates.map(date=>{const day=new Date(`${date}T12:00:00`).getDay();return <section key={date} className={`${date===today?"today ":""}${day===0||day===6?"weekend":""}`.trim()}><h2><strong>{new Intl.DateTimeFormat("sv-SE",{weekday:"short"}).format(new Date(`${date}T12:00:00`))}</strong><b>{Number(date.slice(-2))}</b></h2><div onClick={()=>{if(weekSwipeHandled.current)return;setCreateDate(date);onCreating(true)}}>{(grouped[date]||[]).map(event=>eventCard(event,true))}</div></section>})}</div>:range==="30"?<><div ref={calendarMonthRef} className="calendar-month" onTouchStart={touch=>{if(window.innerWidth<=700){const point=touch.touches[0];monthSwipeStart.current=point?{x:point.clientX,y:point.clientY}:null}}} onTouchEnd={touch=>{const start=monthSwipeStart.current,point=touch.changedTouches[0];monthSwipeStart.current=null;if(!start||!point||window.innerWidth>700)return;const dx=point.clientX-start.x,dy=point.clientY-start.y;if(Math.abs(dx)>=55&&Math.abs(dx)>=Math.abs(dy)*1.25)void changeMonth(dx<0?1:-1)}}><div className="calendar-month-weekdays">{["MÅN","TIS","ONS","TORS","FRE","LÖR","SÖN"].map(day=><b key={day}>{day}</b>)}</div><div className="calendar-month-grid">{monthDates.map(date=>{const dateValue=new Date(`${date}T12:00:00`),day=dateValue.getDay(),items=grouped[date]||[],outside=!date.startsWith(monthKey);return <section key={date} className={`${outside?"outside ":""}${date===today?"today ":""}${day===0||day===6?"weekend ":""}${date===selectedMonthDate?"selected":""}`.trim()} onClick={()=>{if(window.innerWidth<=700)setSelectedMonthDate(date);else{setCreateDate(date);onCreating(true)}}}><time>{dateValue.getDate()}</time><div>{items.slice(0,3).map(event=>{const creator=privateMode?null:members.find(member=>member.uid===event.creatorId),style={"--event-color":rgbaHex(event.color||account.personalColor||colorOptions[0]),"--creator-color":rgbaHex(privateMode?(account.personalColor||colorOptions[0]):(creator?.color||account.personalColor||colorOptions[0]))} as CSSProperties;return <button key={`${event.id}-${date}`} className={event.category==="birthday"?"birthday":""} style={style} onClick={click=>{click.stopPropagation();setViewing(event)}}><i/>{calendarCategory(event.category).icon&&<span>{calendarCategory(event.category).icon}</span>}<strong>{event.title}</strong></button>})}{items.length>3&&<button className="calendar-month-more" onClick={click=>{click.stopPropagation();setSelectedMonthDate(date)}}>+{items.length-3} till</button>}</div></section>})}</div></div><div className="calendar-month-mobile-day"><h2>{calendarDayLabel(selectedMonthDate)}</h2>{(grouped[selectedMonthDate]||[]).length?(grouped[selectedMonthDate]||[]).map(event=>eventCard(event)):<div className="calendar-month-no-events"><span>Inget planerat</span><button onClick={()=>{setCreateDate(selectedMonthDate);onCreating(true)}}><Plus/> LÄGG TILL</button></div>}</div></>:!visible.length?<div className="calendar-empty"><CalendarDays/><strong>Inget planerat här</strong><span>Skönt — eller dags att lägga till något?</span><button onClick={()=>{setCreateDate(today);onCreating(true)}}><Plus/> LÄGG TILL</button></div>:<div className="calendar-agenda">{Object.entries(grouped).map(([date,items])=><section key={date}><h2>{calendarDayLabel(date)}</h2>{items.map(event=>eventCard(event))}</section>)}{range==="agenda"&&agendaLimit<visible.length&&<div ref={agendaLoader} className="calendar-loader">LADDAR FLER…</div>}</div>}
...
function CalendarEditor({event,lists,existing,onClose,onSave,onDelete}:{event:CalendarEvent;lists:BubbsunList[];existing:boolean;onClose:()=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete?:(mode:"single"|"all")=>Promise<void>}){
  const [title,setTitle]=useState(event.title),[date,setDate]=useState(event.date),[time,setTime]=useState(event.time||""),[endTime,setEndTime]=useState(event.endTime||""),[allDay,setAllDay]=useState(event.allDay===true),[category,setCategory]=useState(event.category??""),[color,setColor]=useState(event.color||colorOptions[0]),[repeat,setRepeat]=useState(Boolean(event.recurrenceDays?.length)),[repeatDays,setRepeatDays]=useState<number[]>(event.recurrenceDays||[]),[forever,setForever]=useState(event.recurrenceDays?.length?event.recurrenceForever!==false:true),[until,setUntil]=useState(event.recurrenceUntil||""),[note,setNote]=useState(event.note||""),[linkedListIds,setLinkedListIds]=useState<string[]>((event.linkedListIds||[]).slice(0,3)),[reminderMinutes,setReminderMinutes]=useState(event.reminderMinutes||0),[busy,setBusy]=useState(false),[deleteOpen,setDeleteOpen]=useState(false);
  ...
  return <div className="modal-backdrop"><div className="modal calendar-editor"><button className="calendar-editor-close" aria-label="Stäng" onClick={onClose}><X/></button><CalendarDays/><h2>{existing?"REDIGERA HÄNDELSE":"NY HÄNDELSE"}</h2>
    ...
    <label>KATEGORI<select value={category} onChange={e=>setCategory(e.target.value)}>{calendarCategories.map(value=><option key={value.id||"none"} value={value.id}>{value.icon?`${value.icon} `:""}{value.label}</option>)}</select></label>
    ...
    <div className="calendar-editor-actions">{onDelete&&<button className="calendar-delete" onClick={()=>setDeleteOpen(true)}><Trash2/> TA BORT</button>}<button className="cancel" onClick={onClose}>AVBRYT</button><button disabled={busy||!title.trim()||!date||(!allDay&&Boolean(time&&endTime&&endTime<=time))||(repeat&&!repeatDays.length)||(repeat&&!forever&&!until)} onClick={async()=>{setBusy(true);await requestNotifications();await onSave({...event,title:title.trim(),date,time:allDay?"":time,endTime:allDay?"":endTime,allDay,category,color,recurrenceDays:repeat?repeatDays:[],recurrenceForever:repeat&&forever,recurrenceUntil:repeat&&!forever?until:"",note:note.trim(),linkedListIds,reminderMinutes,updatedAt:Date.now()});setBusy(false)}}>{busy?"SPARAR…":existing?"SPARA":"SKAPA"}</button></div>
