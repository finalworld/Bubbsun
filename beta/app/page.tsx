...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>}){
  const now=new Date(),[monthOffset,setMonthOffset]=useState(0),[spaceOpen,setSpaceOpen]=useState(false),[settingsOpen,setSettingsOpen]=useState(false),[transferOpen,setTransferOpen]=useState(false),[editing,setEditing]=useState<BudgetEntry|null|undefined>(undefined),[confirmDelete,setConfirmDelete]=useState<BudgetEntry|null>(null),[view,setView]=useState<"overview"|"recurring">("overview"),[createType,setCreateType]=useState<"income"|"expense">("expense");
  ...
  return <section className="content budget-page">
    <header className="budget-intro"><WalletCards/><div><h1>BUDGET</h1><p>En enkel överblick över pengarna, privat eller tillsammans.</p></div></header>
    <nav className="budget-tabs" aria-label="Budgetmeny"><div className="budget-tab-links"><button className={view==="overview"?"selected":""} onClick={()=>setView("overview")}><WalletCards/>ÖVERSIKT</button><button onClick={()=>{setCreateType("income");setEditing(undefined);onCreating(true)}}><b>+</b> NY INKOMST</button><button onClick={()=>{setCreateType("expense");setEditing(undefined);onCreating(true)}}><b>−</b> NY UTGIFT</button><button onClick={()=>setTransferOpen(true)}><ArrowLeftRight/> ÖVERFÖRING</button><button className={view==="recurring"?"selected":""} onClick={()=>setView("recurring")}><History/> ÅTERKOMMANDE</button></div><div className="budget-tab-tools"><div className="calendar-space-wrap budget-space"><button className="calendar-space-button" onClick={()=>setSpaceOpen(value=>!value)}>{privateMode?<LockKeyhole/>:<GroupIcon id={activeGroup?.iconId}/>}<span><small>{privateMode?"PRIVAT":"GRUPP"}</small><strong>{privateMode?"Bara jag":activeGroup?.name||"Välj grupp"}</strong></span><ChevronDown/></button>{spaceOpen&&<div className="calendar-space-menu"><button className={privateMode?"selected":""} onClick={()=>{onMode(true);setSpaceOpen(false)}}><LockKeyhole/><strong>Privat</strong>{privateMode&&<Check/>}</button>{memberships.map(item=><button key={item.groupId} className={!privateMode&&item.groupId===account.activeGroupId?"selected":""} onClick={()=>{onSwitchGroup(item.groupId);setSpaceOpen(false)}}><GroupIcon id={groups[item.groupId]?.iconId}/><strong>{groups[item.groupId]?.name||"Grupp"}</strong>{!privateMode&&item.groupId===account.activeGroupId&&<Check/>}</button>)}</div>}</div><button className="budget-settings-trigger" aria-label="Budgetinställningar" title="Budgetinställningar" onClick={()=>setSettingsOpen(true)}><Settings/></button></div></nav>
    {view==="overview"?<>
...
function BudgetAccountOverview({settings,entries}:{settings:BudgetSettings;entries:BudgetEntry[]}){if(!settings.banks.length)return null;const accountTotal=(id:string)=>entries.reduce((sum,entry)=>entry.type==="transfer"?sum+(entry.toAccountId===id?entry.amount:0)-(entry.fromAccountId===id?entry.amount:0):entry.accountId===id?sum+(entry.type==="income"?entry.amount:-entry.amount):sum,0);return <section className="budget-accounts-overview"><header><span><small>DIN MÅNADSÖVERSIKT</small><h2>BANKER & KONTON</h2></span><b>{settings.banks.reduce((sum,bank)=>sum+bank.accounts.reduce((total,item)=>total+accountTotal(item.id),0),0)} kr totalt</b></header><div>{settings.banks.map(bank=><article key={bank.id}><header><span>▣</span><strong>{bank.name}</strong><b>{budgetMoney(bank.accounts.reduce((sum,item)=>sum+accountTotal(item.id),0))}</b></header><div>{bank.accounts.map(account=>{const total=accountTotal(account.id);return <p key={account.id}><span><i>{budgetAccountIcon(account.icon)}</i><strong>{account.name}</strong></span><b className={total<0?"expense":"income"}>{budgetMoney(total)}</b></p>})}</div></article>)}</div></section>}
