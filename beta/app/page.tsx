...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,groupBudgetSettings,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset,onClearMoney}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;groupBudgetSettings:Record<string,BudgetSettings>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
...
function BudgetSettingsEditor({settings,privateMode,groups,groupBudgetSettings,onClose,onSave,onReset,onClearMoney}:{settings:BudgetSettings;privateMode:boolean;groups:Record<string,Group>;groupBudgetSettings:Record<string,BudgetSettings>;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  const [banks,setBanks]=useState(settings.banks),[categoryBudgets,setCategoryBudgets]=useState(settings.categoryBudgets||{}),[goals,setGoals]=useState(settings.savingsGoals||[]),[resetStep,setResetStep]=useState(0),[resetText,setResetText]=useState(""),[moneyResetStep,setMoneyResetStep]=useState(0),[linkValue,setLinkValue]=useState("");
  ...
  const addLinkedAccount=()=>{const [groupId,accountId]=linkValue.split("::"),source=linkOptions.find(item=>item.groupId===groupId&&item.account.id===accountId);if(!source||banks.some(bank=>bank.accounts.some(item=>item.linkedGroupId===groupId&&item.linkedAccountId===accountId)))return;setBanks(old=>{const linked=old.find(bank=>bank.id==="linked-group-accounts"),account={...source.account,id:crypto.randomUUID(),linkedGroupId:groupId,linkedAccountId:accountId};return linked?old.map(bank=>bank.id===linked.id?{...bank,accounts:[...bank.accounts,account]}:bank):[...old,{id:"linked-group-accounts",name:"Delade gruppkonton",accounts:[account]}]});setLinkValue("")};
...
function AuthenticatedApp() {
  ...
  const persistBudgetSettings=async(settings:BudgetSettings)=>{if(!user||!account)return;if(privateMode){const sourceUpdates=new Map<string,BudgetSettings>();for(const alias of settings.banks.flatMap(bank=>bank.accounts).filter(item=>item.linkedGroupId&&item.linkedAccountId)){const current=sourceUpdates.get(alias.linkedGroupId!)||groupBudgetSettings[alias.linkedGroupId!];if(!current)continue;sourceUpdates.set(alias.linkedGroupId!,{...current,banks:current.banks.map(bank=>({...bank,accounts:bank.accounts.map(item=>item.id===alias.linkedAccountId?{...item,name:alias.name,icon:alias.icon,openingBalance:alias.openingBalance,reconciledBalance:alias.reconciledBalance,reconciledAt:alias.reconciledAt}:item)})),updatedAt:Date.now()})}await Promise.all([...sourceUpdates].map(([groupId,value])=>saveBudgetSettings(groupId,value)));await savePrivateBudgetSettings(user.uid,settings)}else if(account.activeGroupId)await saveBudgetSettings(account.activeGroupId,settings)};
