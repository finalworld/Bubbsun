...
const budgetOccurrencePaid=(entry:BudgetEntry,occurrenceDate:string,today=calendarDateKey(new Date()))=>entry.type==="transfer"&&Boolean(entry.recurrence)?occurrenceDate<=today:entry.status!=="planned"||(entry.autoPay===true&&occurrenceDate<=today);
...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset,onClearMoney}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  ...
  return <section className="content budget-page">
    ...
    <BudgetAccountOverview settings={settings} entries={paidEntries.map(item=>item.entry)} onSelect={setSelectedAccountId}/><section className="budget-list"><header className="budget-list-tools"><h2>MÅNADENS POSTER</h2><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Sök poster…"/><select value={filter} onChange={event=>setFilter(event.target.value as typeof filter)}><option value="all">Alla</option><option value="income">Inkomster</option><option value="expense">Utgifter</option><option value="transfer">Överföringar</option><option value="planned">Planerade</option></select></header>{visibleEntries.length?visibleEntries.map(({entry,occurrenceDate})=>{const paid=budgetOccurrencePaid(entry,occurrenceDate,todayKey),overdue=!paid&&occurrenceDate<todayKey;return <button key={`${entry.id}-${occurrenceDate}`} className={`budget-row ${!paid?"planned":""} ${overdue?"overdue":""}`} onClick={()=>{if(entry.type==="transfer"){setEditingTransfer(entry);setTransferOpen(true)}else setEditing(entry)}}><span className={entry.type}>{entry.type==="income"?"+":entry.type==="expense"?"−":"⇄"}</span><span><strong>{entry.title}</strong><small>{overdue?"⚠ Försenad · ":!paid?(entry.autoPay?"◷ Betalas automatiskt · ":"◷ Planerad · "):entry.autoPay?"✓ Automatiskt betald · ":"✓ Betald · "}{entry.type==="transfer"?`${accountName(entry.fromAccountId)} → ${accountName(entry.toAccountId)}`:<>{entry.category}{entry.subcategory?` · ${entry.subcategory}`:""} · {accountName(entry.accountId)}</>} · {new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short"}).format(new Date(`${occurrenceDate}T12:00:00`))}{!privateMode&&<> · {entry.creatorName}</>}</small></span><b className={entry.type}>{entry.type==="income"?"+ ":entry.type==="expense"?"− ":""}{budgetMoney(entry.amount)}</b><ChevronRight/></button>}):<div className="budget-empty"><WalletCards/><h3>Inga poster matchar</h3></div>}</section></>:<section className="budget-list budget-recurring-list"><header><div><small>AUTOMATISKA POSTER</small><h2>ÅTERKOMMANDE</h2></div><b>{entries.filter(entry=>Boolean(entry.recurrence)).length} st</b></header>{entries.filter(entry=>Boolean(entry.recurrence)).sort((a,b)=>a.date.localeCompare(b.date)).map(entry=><button key={entry.id} className="budget-row" onClick={()=>{if(entry.type==="transfer"){setEditingTransfer(entry);setTransferOpen(true)}else setEditing(entry)}}><span className={entry.type}>{entry.type==="income"?"+":entry.type==="expense"?"−":"⇄"}</span><span><strong>{entry.title}</strong><small>{entry.type==="transfer"?"Överföring":entry.category} · {entry.recurrence==="weekly"?`varje ${new Intl.DateTimeFormat("sv-SE",{weekday:"long"}).format(new Date(`${entry.date}T12:00:00`))}`:`varje månad den ${Number(entry.date.slice(-2))}:e`}{entry.autoPay?" · betalas automatiskt":""}{!privateMode&&<> · {entry.creatorName}</>}</small></span><b className={entry.type}>{entry.type==="income"?"+ ":entry.type==="expense"?"− ":""}{budgetMoney(entry.amount)}</b><ChevronRight/></button>)}{!entries.some(entry=>Boolean(entry.recurrence))&&<div className="budget-empty"><History/><h3>Inga återkommande poster</h3><p>Markera “Återkommer varje månad” när du skapar en post.</p></div>}</section>}
  ...
  return (
    ...
    >
      <section className="modal budget-editor">
        ...
        {recurring&&status==="planned"&&<label className="budget-recurring budget-auto-pay"><input type="checkbox" checked={autoPay} onChange={event=>setAutoPay(event.target.checked)}/><span><strong>Genomför automatiskt på datumet</strong><small>Posten räknas som betald och saldot ändras automatiskt den dagen.</small></span></label>}
