      ...
      <aside className={`drawer ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="drawer-footer">
          ...
          <small className="drawer-version-text">
            Bubbsun v0.850 · Web Edition Beta
          </small>
        </div>
      </aside>
    </>
...
function CalendarPage({events,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onOpenList}:{events:CalendarEvent[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete:(event:CalendarEvent)=>Promise<void>;onOpenList:(list:BubbsunList)=>void}){
  const [range,setRange]=useState<CalendarRange>(()=>(localStorage.getItem("bubbsun-calendar-range") as CalendarRange)||"7"),[spaceOpen,setSpaceOpen]=useState(false),[viewing,setViewing]=useState<CalendarOccurrence|null>(null),[editing,setEditing]=useState<CalendarEvent|null>(null),[createDate,setCreateDate]=useState(calendarDateKey(new Date())),[weekOffset,setWeekOffset]=useState(0),[monthOffset,setMonthOffset]=useState(0),[selectedMonthDate,setSelectedMonthDate]=useState(calendarDateKey(new Date())),[agendaLimit,setAgendaLimit]=useState(20);const agendaLoader=useRef<HTMLDivElement>(null),calendarWeekRef=useRef<HTMLDivElement>(null),calendarMonthRef=useRef<HTMLDivElement>(null),weekSwipeStart=useRef<{x:number;y:number;dragging:boolean}|null>(null),weekSwipeHandled=useRef(false),weekAnimating=useRef(false),monthSwipeStart=useRef<{x:number;y:number;dragging:boolean}|null>(null),monthAnimating=useRef(false);
  ...
  const moveCarousel=(start:{x:number;y:number;dragging:boolean}|null,element:HTMLDivElement|null,x:number,y:number)=>{if(!start||!element)return;const dx=x-start.x,dy=y-start.y;if(!start.dragging){if(Math.abs(dx)<8&&Math.abs(dy)<8)return;if(Math.abs(dy)>=Math.abs(dx)){start.x=Number.NaN;return}start.dragging=true;weekSwipeHandled.current=true}if(Number.isNaN(start.x)||!start.dragging)return;element.style.transition="none";element.style.transform=`translate3d(calc(-33.333333% + ${dx}px),0,0)`};
  const finishCarousel=(start:{x:number;y:number;dragging:boolean}|null,element:HTMLDivElement|null,x:number,y:number,change:(direction:-1|1)=>Promise<void>)=>{if(!start||!element||Number.isNaN(start.x)||!start.dragging)return;const dx=x-start.x,dy=y-start.y;if(Math.abs(dx)>=55&&Math.abs(dx)>=Math.abs(dy)*1.15)void change(dx<0?1:-1);else{element.style.transition="transform 180ms ease-out";element.style.transform="translate3d(-33.333333%,0,0)"}window.setTimeout(()=>{weekSwipeHandled.current=false},450)};
