      ...
      <aside className={`drawer ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="drawer-footer">
          ...
          <small className="drawer-version-text">
            Bubbsun v0.831 · Web Edition Beta
          </small>
        </div>
      </aside>
    </>
...
function CalendarEventCard({event,lists,members,privateMode,account,onClose,onOpenList,onEdit}:{event:CalendarOccurrence;lists:BubbsunList[];members:Membership[];privateMode:boolean;account:Account;onClose:()=>void;onOpenList:(list:BubbsunList)=>void;onEdit:()=>void}){
  ...
  return <div className="modal-backdrop"><article className={`modal calendar-event-card${event.category==="birthday"?" birthday":""}`} style={style}><button className="calendar-card-close" aria-label="Stäng" onClick={onClose}><X/></button><div className="calendar-card-icon">{category.icon||"📅"}</div><small>{category.label||"Kalenderpost"}</small><h2>{event.title}</h2><div className="calendar-card-facts"><p><CalendarDays/><span><small>DATUM</small><strong>{dateLabel}</strong></span></p><p><Bell/><span><small>TID & PÅMINNELSE</small><strong>{timeLabel} · {reminderLabel}</strong></span></p>{repeat&&<p><History/><span><small>ÅTERKOMMANDE</small><strong>{event.recurrenceForever?"Återkommer för alltid":event.recurrenceUntil?`Till ${event.recurrenceUntil}`:"Ja"}</strong></span></p>}</div>{event.note&&<section className="calendar-card-note"><small>ANTECKNING</small><p>{event.note}</p></section>}{linked.length>0&&<section className="calendar-card-lists"><small>KOPPLADE LISTOR</small>{linked.map(list=><button key={list.id} onClick={()=>onOpenList(list)}><span>🔗</span><strong>{list.name}</strong><ChevronRight/></button>)}</section>}<p className="calendar-card-creator" style={{borderColor:"var(--creator-color)"}}>Skapad av {creatorName}</p><button className="calendar-card-edit" onClick={onEdit}><Pencil/> REDIGERA</button></article></div>
}
