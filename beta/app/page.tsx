      ...
      <aside className={`drawer ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="drawer-footer">
          ...
          <small className="drawer-version-text">
            Bubbsun v0.900 · Web Edition Beta
          </small>
        </div>
      </aside>
    </>
...
function CalendarPage({events,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onOpenList,openEventId,onEventOpened}:{events:CalendarEvent[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete:(event:CalendarEvent)=>Promise<void>;onOpenList:(list:BubbsunList)=>void;openEventId?:string;onEventOpened?:()=>void}){
  ...
  return <section className="content calendar-page">
    ...
    {editor&&<CalendarEditor
      ...
      onSave={async next=>{await onSave(next);setEditing(null);setCreateDate(today);onCreating(false)}}
...
function CalendarEditor({event,lists,existing,onClose,onSave,onDelete}:{event:CalendarEvent;lists:BubbsunList[];existing:boolean;onClose:()=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete?:(mode:"single"|"all")=>Promise<void>}){
  const [title,setTitle]=useState(event.title),[date,setDate]=useState(event.date),[time,setTime]=useState(event.time||""),[endTime,setEndTime]=useState(event.endTime||""),[allDay,setAllDay]=useState(event.allDay===true),[category,setCategory]=useState(event.category??""),[birthYear,setBirthYear]=useState(event.birthYear?String(event.birthYear):""),[color,setColor]=useState(event.color||colorOptions[0]),[repeat,setRepeat]=useState(event.recurrenceType==="yearly"||Boolean(event.recurrenceDays?.length)),[repeatDays,setRepeatDays]=useState<number[]>(event.recurrenceDays||[]),[forever,setForever]=useState(event.recurrenceDays?.length?event.recurrenceForever!==false:true),[until,setUntil]=useState(event.recurrenceUntil||""),[note,setNote]=useState(event.note||""),[linkedListIds,setLinkedListIds]=useState<string[]>((event.linkedListIds||[]).slice(0,3)),[reminderMinutes,setReminderMinutes]=useState(event.reminderMinutes||0),[busy,setBusy]=useState(false),[deleteOpen,setDeleteOpen]=useState(false);
...
function RecipeShareControl({recipe}:{recipe:Recipe}){
  const [open,setOpen]=useState(false),url=recipePublicUrl(recipe);
  return <><button className="recipe-share-button" type="button" onClick={()=>setOpen(true)}><Share2/> DELA</button>{open&&<div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}><div className="modal share-list-modal recipe-share-modal"><button className="modal-close recipe-share-close" aria-label="Stäng" onClick={()=>setOpen(false)}><X/></button><Share2 className="share-modal-icon"/><h2>DELA RECEPT</h2><p>Alla med länken kan läsa det här publika receptet utan konto.</p><input readOnly value={url} onFocus={event=>event.currentTarget.select()}/><div className="share-site-actions"><button onClick={async()=>{await navigator.clipboard.writeText(url);window.alert("Länken är kopierad ✓")}}><Copy/> KOPIERA LÄNK</button>{typeof navigator.share==="function"&&<button onClick={()=>void navigator.share({title:recipe.title,text:`${recipe.title} – ett recept på Bubbsun`,url})}><Share2/> DELA</button>}</div></div></div>}</>;
}
...
function RecipeEditor({recipe,account,lists,onClose,onSave,onDelete}:{recipe?:Recipe;account:Account;lists:BubbsunList[];onClose:()=>void;onSave:(recipe:Recipe)=>Promise<void>;onDelete?:()=>Promise<void>}){
  const [title,setTitle]=useState(recipe?.title||""),[category,setCategory]=useState(recipe?.category||""),[subcategory,setSubcategory]=useState(recipe?.subcategory||""),[isPublic,setIsPublic]=useState(recipe?.isPublic===true),[image,setImage]=useState(recipe?.image||""),[servings,setServings]=useState(recipe?.servings||4),[minutes,setMinutes]=useState(recipe?.minutes||0),[ingredients,setIngredients]=useState(recipe?.ingredients||[{id:crypto.randomUUID(),amount:"",unit:"",name:""}]),[instructions,setInstructions]=useState(recipe?.instructions||""),[note,setNote]=useState(recipe?.note||""),[linkedListId,setLinkedListId]=useState(recipe?.linkedListId||""),[busy,setBusy]=useState(false),[confirmDelete,setConfirmDelete]=useState(false);
...
function RecipesPage({recipes,lists,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onAddToList,openRecipeId,onRecipeOpened}:{recipes:Recipe[];lists:BubbsunList[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>Promise<void>;onSave:(recipe:Recipe)=>Promise<void>;onDelete:(recipe:Recipe)=>Promise<void>;onAddToList:(recipe:Recipe,listId:string)=>Promise<void>;openRecipeId?:string;onRecipeOpened?:()=>void}){
  ...
  useEffect(()=>setRecipePage(1),[search,category,subcategory,privateMode,account.activeGroupId]);
  return <section className="content recipes-page">
    <div className="recipe-cookbook-primary"><label className="recipe-cookbook-search"><Search/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Sök recept eller skapare"/></label><div className="recipe-scope-wrap">
      ...
      {spaceOpen&&<div className="recipe-scope-menu"><button className={privateMode?"selected":""} onClick={()=>{onMode(true);setSpaceOpen(false)}}><LockKeyhole/><strong>Privat</strong>{privateMode&&<Check/>}</button>{memberships.map(item=><button key={item.groupId} className={!privateMode&&item.groupId===account.activeGroupId?"selected":""} onClick={()=>{void onSwitchGroup(item.groupId);setSpaceOpen(false)}}><GroupIcon id={groups[item.groupId]?.iconId}/><strong>{groups[item.groupId]?.name||"Grupp"}</strong>{!privateMode&&item.groupId===account.activeGroupId&&<Check/>}</button>)}{!memberships.length&&<button type="button" className="group-empty-state" onClick={()=>{setSpaceOpen(false);window.dispatchEvent(new CustomEvent("bubbsun:navigate",{detail:"people"}))}}><Users/><span><strong>Ingen grupp ännu</strong><small>Skapa en grupp eller gå med i en på sidan Användare &amp; grupper.</small><b>TILL ANVÄNDARE &amp; GRUPPER</b></span><ChevronRight/></button>}</div>}
    </div></div>
  ...
  return (
    <main className={`app-shell theme-${activeTheme.id}`} style={themeStyle}>
      ...
      <Header
        ...
        onAdd={() => page === "notes" ? setAddingNote(true) : page === "calendar" ? setAddingCalendar(true) : page === "recipes" ? setAddingRecipe(true) : setAdding(true)}
...
function usePublicRecipeMetadata(recipe:Recipe|null|undefined){
  useEffect(()=>{
    ...
    const title=recipe?`${recipe.title} – recept | Bubbsun`:"Receptet hittades inte | Bubbsun",description=recipe?`${recipeCategoryLabel(recipe)} av ${recipe.creatorName}. ${recipe.servings} portioner${recipe.minutes?`, ${recipe.minutes} minuter`:""}.`:"Det publika receptet finns inte längre.";
    ...
    if(recipe){const script=document.createElement("script");script.id="public-recipe-jsonld";script.type="application/ld+json";script.text=JSON.stringify({"@context":"https://schema.org","@type":"Recipe",name:recipe.title,image:recipe.image?[recipe.image]:undefined,author:{"@type":"Person",name:recipe.creatorName},recipeCategory:recipe.category,recipeCuisine:recipe.subcategory,recipeYield:`${recipe.servings} portioner`,totalTime:recipe.minutes?`PT${recipe.minutes}M`:undefined,recipeIngredient:recipe.ingredients.map(item=>[[item.amount,item.unit].filter(Boolean).join(" "),item.name].filter(Boolean).join(" ")),recipeInstructions:recipeInstructionSteps(recipe.instructions).map(text=>({"@type":"HowToStep",text}))});document.getElementById(script.id)?.remove();document.head.appendChild(script)}
  },[recipe]);
}

function PublicRecipeArticle({recipe}:{recipe:Recipe}){return <article className="recipe-view public-recipe-view" style={{"--recipe-creator":"var(--theme-accent)"} as CSSProperties}>{recipe.image?<img className="recipe-hero" src={recipe.image} alt={recipe.title}/>:<div className="recipe-hero fallback">🍲</div>}<small>{recipeCategoryLabel(recipe)}</small><h1>{recipe.title}</h1><div className="recipe-view-share"><RecipeShareControl recipe={recipe}/></div><div className="recipe-facts"><span>🍽️ {recipe.servings} portioner</span>{recipe.minutes>0&&<span>⏱️ {recipe.minutes} minuter</span>}</div><div className="recipe-creator-line">Skapad av {recipe.creatorName}</div><section><h3>INGREDIENSER</h3><ul>{recipe.ingredients.map(item=><li key={item.id}><b>{[item.amount,item.unit].filter(Boolean).join(" ")}</b> {item.name}</li>)}</ul></section><section><h3>GÖR SÅ HÄR</h3><div className="recipe-instructions">{recipeInstructionSteps(recipe.instructions).map((step,index)=><p key={index}><b>{index+1}</b><span>{step}</span></p>)}</div></section>{recipe.note&&<aside><b>ANTECKNING</b><p>{recipe.note}</p></aside>}<footer><a href="/recept">← Upptäck fler recept</a></footer></article>}
