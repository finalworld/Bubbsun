      ...
      <aside className={`drawer ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="drawer-footer">
          ...
          <small className="drawer-version-text">
            Bubbsun v0.824 · Web Edition Beta
          </small>
        </div>
      </aside>
    </>
...
function CalendarPage({events,privateMode,account,memberships,groups,members,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete}:{events:CalendarEvent[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;members:Membership[];creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete:(event:CalendarEvent)=>Promise<void>}){
  const [range,setRange]=useState<CalendarRange>(()=>(localStorage.getItem("bubbsun-calendar-range") as CalendarRange)||"7"),[spaceOpen,setSpaceOpen]=useState(false),[editing,setEditing]=useState<CalendarEvent|null>(null),[createDate,setCreateDate]=useState(calendarDateKey(new Date())),[weekOffset,setWeekOffset]=useState(0),[agendaLimit,setAgendaLimit]=useState(20);const agendaLoader=useRef<HTMLDivElement>(null);
  ...
  const activeGroup=groups[account.activeGroupId];
  const editor=editing||creating?editing||{id:crypto.randomUUID(),title:"",date:createDate,time:"",endTime:"",allDay:false,category:"other",color:account.personalColor??colorOptions[0],note:"",creatorId:account.uid,creatorName:account.displayName,createdAt:Date.now(),updatedAt:Date.now()}:null;
  const eventCard=(event:CalendarOccurrence,compact=false)=>{const creator=privateMode?null:members.find(member=>member.uid===event.creatorId),color=rgbaHex(event.color||creator?.color||account.personalColor||colorOptions[0]),creatorColor=rgbaHex(privateMode?(account.personalColor||colorOptions[0]):(creator?.color||account.personalColor||colorOptions[0])),category=calendarCategory(event.category),birthday=event.category==="birthday",timeLabel=event.allDay||!event.time?"HELA DAGEN":event.endTime?`${event.time}–${event.endTime}`:event.time,style={"--event-color":color,"--creator-color":creatorColor} as CSSProperties,key=`${event.id}-${event.occurrenceDate}`,open=(click:{stopPropagation:()=>void})=>{click.stopPropagation();setEditing(event)};if(compact)return <button className={`calendar-event compact${birthday?" birthday":""}`} key={key} style={style} onClick={open}><span className="calendar-compact-meta"><b>{category.icon}</b><time>{timeLabel}</time>{Boolean(event.recurrenceDays?.length)&&<small className="calendar-repeat">↻</small>}</span><strong className="calendar-compact-title">{event.title}</strong></button>;return <button className={`calendar-event${birthday?" birthday":""}`} key={key} style={style} onClick={open}><i/><time>{timeLabel}</time><span><strong><b>{category.icon}</b>{event.title}{Boolean(event.recurrenceDays?.length)&&<small className="calendar-repeat">↻</small>}</strong>{event.note&&<small>{event.note}</small>}</span><em aria-hidden="true"/><ChevronRight/></button>};
...
function CalendarEditor({event,existing,onClose,onSave,onDelete}:{event:CalendarEvent;existing:boolean;onClose:()=>void;onSave:(event:CalendarEvent)=>Promise<void>;onDelete?:()=>Promise<void>}){const [title,setTitle]=useState(event.title),[date,setDate]=useState(event.date),[time,setTime]=useState(event.time||""),[endTime,setEndTime]=useState(event.endTime||""),[allDay,setAllDay]=useState(event.allDay===true),[category,setCategory]=useState(event.category||"other"),[color,setColor]=useState(event.color||colorOptions[0]),[repeat,setRepeat]=useState(Boolean(event.recurrenceDays?.length)),[repeatDays,setRepeatDays]=useState<number[]>(event.recurrenceDays||[]),[forever,setForever]=useState(event.recurrenceDays?.length?event.recurrenceForever!==false:true),[until,setUntil]=useState(event.recurrenceUntil||""),[note,setNote]=useState(event.note||""),[busy,setBusy]=useState(false);const weekdayLabels=[[1,"MÅ"],[2,"TI"],[3,"ON"],[4,"TO"],[5,"FR"],[6,"LÖ"],[0,"SÖ"]] as const;return <div className="modal-backdrop"><div className="modal calendar-editor"><button className="calendar-editor-close" aria-label="Stäng" onClick={onClose}><X/></button><CalendarDays/><h2>{existing?"REDIGERA HÄNDELSE":"NY HÄNDELSE"}</h2><label>VAD HÄNDER?<input autoFocus value={title} maxLength={80} onChange={e=>setTitle(e.target.value)} placeholder="Till exempel tandläkaren"/></label><label>KATEGORI<select value={category} onChange={e=>setCategory(e.target.value)}>{calendarCategories.map(value=><option key={value.id} value={value.id}>{value.icon} {value.label}</option>)}</select></label><label>FÄRG<div className="calendar-color-picker">{colorOptions.map(value=><button type="button" key={value} className={color===value?"selected":""} style={{"--choice-color":rgbaHex(value)} as CSSProperties} onClick={()=>setColor(value)} aria-label="Välj färg"/>)}</div></label><div><label>DATUM<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label className="calendar-all-day"><input type="checkbox" checked={allDay} onChange={e=>setAllDay(e.target.checked)}/><span>HELDAG</span></label></div>{!allDay&&<div><label>FRÅN<input type="time" value={time} onChange={e=>setTime(e.target.value)}/></label><label>TILL<input type="time" value={endTime} min={time} onChange={e=>setEndTime(e.target.value)}/></label></div>}<section className="calendar-recurrence"><label className="calendar-repeat-toggle"><input type="checkbox" checked={repeat} onChange={e=>{const enabled=e.target.checked;setRepeat(enabled);if(enabled&&!repeatDays.length)setRepeatDays([new Date(`${date}T12:00:00`).getDay()])}}/><span>↻ ÅTERKOMMANDE</span></label>{repeat&&<><div className="calendar-weekdays">{weekdayLabels.map(([value,label])=><button type="button" key={value} className={repeatDays.includes(value)?"selected":""} onClick={()=>setRepeatDays(current=>current.includes(value)?current.filter(day=>day!==value):[...current,value])}>{label}</button>)}</div><div className="calendar-repeat-end"><label><input type="radio" checked={forever} onChange={()=>setForever(true)}/> FÖR ALLTID</label><label><input type="radio" checked={!forever} onChange={()=>setForever(false)}/> TILL DATUM</label>{!forever&&<input type="date" min={date} value={until} onChange={e=>setUntil(e.target.value)}/>}</div></>}</section><label>ANTECKNING <small>(valfritt)</small><textarea value={note} maxLength={300} onChange={e=>setNote(e.target.value)} placeholder="Något som är bra att komma ihåg…"/></label><div className="calendar-editor-actions">{onDelete&&<button className="calendar-delete" onClick={()=>void onDelete()}><Trash2/> TA BORT</button>}<button className="cancel" onClick={onClose}>AVBRYT</button><button disabled={busy||!title.trim()||!date||(!allDay&&Boolean(time&&endTime&&endTime<=time))||(repeat&&!repeatDays.length)||(repeat&&!forever&&!until)} onClick={async()=>{setBusy(true);await onSave({...event,title:title.trim(),date,time:allDay?"":time,endTime:allDay?"":endTime,allDay,category,color,recurrenceDays:repeat?repeatDays:[],recurrenceForever:repeat&&forever,recurrenceUntil:repeat&&!forever?until:"",note:note.trim(),updatedAt:Date.now()});setBusy(false)}}>{busy?"SPARAR…":existing?"SPARA":"SKAPA"}</button></div></div></div>}
