...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>}){
...
function BudgetSettingsEditor({settings,onClose,onSave,onReset}:{settings:BudgetSettings;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>}){
  const [banks,setBanks]=useState(settings.banks),[categoryBudgets,setCategoryBudgets]=useState(settings.categoryBudgets||{}),[goals,setGoals]=useState(settings.savingsGoals||[]),[resetStep,setResetStep]=useState(0),[resetText,setResetText]=useState("");
