    ...
    <header className={`topbar text-topbar text-header-v2 ${scrolled ? "is-scrolled" : ""}`}>
      <div className="topbar-inner">
        ...
        {wallet ? <div className="header-game-wallet"><span><small>SALDO</small><strong>{wallet.balance.toLocaleString("sv-SE")} Bb</strong></span><span><small>VALV</small><strong>{wallet.vault.toLocaleString("sv-SE")} Bb</strong></span></div> : mode === "add" ? (
          <div className="header-add-actions">{tabTitle==="Budget"&&<button className="header-transfer" aria-label="Ny överföring" title="Ny överföring" onClick={()=>window.dispatchEvent(new Event("bubbsun:new-budget-transfer"))}><ArrowLeftRight/></button>}<button
