...
function BudgetPage({entries,settings,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete,onSaveSettings,onReset,onClearMoney}:{entries:BudgetEntry[];settings:BudgetSettings;privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>;onSaveSettings:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  ...
  const monthDate=new Date(now.getFullYear(),now.getMonth()+monthOffset,1),monthKey=`${monthDate.getFullYear()}-${String(monthDate.getMonth()+1).padStart(2,"0")}`,monthNumber=monthDate.getFullYear()*12+monthDate.getMonth(),expandMonth=(targetDate:Date)=>{const key=`${targetDate.getFullYear()}-${String(targetDate.getMonth()+1).padStart(2,"0")}`,number=targetDate.getFullYear()*12+targetDate.getMonth();return entries.flatMap(entry=>{const start=new Date(`${entry.date}T12:00:00`),startNumber=start.getFullYear()*12+start.getMonth();if(entry.recurrence==="monthly"&&number>=startNumber){const day=Math.min(start.getDate(),new Date(targetDate.getFullYear(),targetDate.getMonth()+1,0).getDate());return[{entry,occurrenceDate:`${key}-${String(day).padStart(2,"0")}`}]}return entry.date.startsWith(key)?[{entry,occurrenceDate:entry.date}]:[]})},monthEntries=expandMonth(monthDate).sort((a,b)=>new Date(`${b.occurrenceDate}T12:00:00`).getTime()-new Date(`${a.occurrenceDate}T12:00:00`).getTime()||(b.entry.createdAt||0)-(a.entry.createdAt||0)),paidEntries=monthEntries.filter(item=>item.entry.status!=="planned"),income=paidEntries.filter(item=>item.entry.type==="income").reduce((sum,item)=>sum+item.entry.amount,0),expense=paidEntries.filter(item=>item.entry.type==="expense").reduce((sum,item)=>sum+item.entry.amount,0),previousDate=new Date(monthDate.getFullYear(),monthDate.getMonth()-1,1),previousEntries=expandMonth(previousDate).filter(item=>item.entry.status!=="planned"),previousExpense=previousEntries.filter(item=>item.entry.type==="expense").reduce((sum,item)=>sum+item.entry.amount,0),visibleEntries=monthEntries.filter(({entry})=>(filter==="all"||(filter==="planned"?entry.status==="planned":entry.type===filter))&&`${entry.title} ${entry.category} ${entry.subcategory||""}`.toLocaleLowerCase("sv-SE").includes(search.trim().toLocaleLowerCase("sv-SE"))),activeGroup=groups[account.activeGroupId];
  const accountName=(id?:string)=>id===budgetUnassignedAccountId?"Ej placerat":settings.banks.flatMap(bank=>bank.accounts.map(item=>({id:item.id,label:`${bank.name} · ${item.name}`}))).find(item=>item.id===id)?.label||"Ej valt konto";
  return <section className="content budget-page">
    ...
    <section className="budget-insights"><article><small>JÄMFÖRT MED FÖRRA MÅNADEN</small><strong className={expense>previousExpense?"expense":"income"}>{expense===previousExpense?"Samma utgifter":`${budgetMoney(Math.abs(expense-previousExpense))} ${expense>previousExpense?"mer":"mindre"}`}</strong></article><article><small>PLANERAT</small><strong>{monthEntries.filter(item=>item.entry.status==="planned").length} poster · {budgetMoney(monthEntries.filter(item=>item.entry.status==="planned").reduce((sum,item)=>sum+item.entry.amount,0))}</strong></article></section>
    ...
    <BudgetAccountOverview settings={settings} entries={paidEntries.map(item=>item.entry)} onSelect={setSelectedAccountId}/><section className="budget-list"><header className="budget-list-tools"><h2>MÅNADENS POSTER</h2><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Sök poster…"/><select value={filter} onChange={event=>setFilter(event.target.value as typeof filter)}><option value="all">Alla</option><option value="income">Inkomster</option><option value="expense">Utgifter</option><option value="transfer">Överföringar</option><option value="planned">Planerade</option></select></header>{visibleEntries.length?visibleEntries.map(({entry,occurrenceDate})=>{const overdue=entry.status==="planned"&&occurrenceDate<calendarDateKey(now);return <button key={`${entry.id}-${occurrenceDate}`} className={`budget-row ${entry.status==="planned"?"planned":""} ${overdue?"overdue":""}`} onClick={()=>{if(entry.type==="transfer"){setEditingTransfer(entry);setTransferOpen(true)}else setEditing(entry)}}><span className={entry.type}>{entry.type==="income"?"+":entry.type==="expense"?"−":"⇄"}</span><span><strong>{entry.title}</strong><small>{overdue?"⚠ Försenad · ":entry.status==="planned"?"◷ Planerad · ":"✓ Betald · "}{entry.type==="transfer"?`${accountName(entry.fromAccountId)} → ${accountName(entry.toAccountId)}`:<>{entry.category}{entry.subcategory?` · ${entry.subcategory}`:""} · {accountName(entry.accountId)}</>} · {new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short"}).format(new Date(`${occurrenceDate}T12:00:00`))} · {entry.creatorName}</small></span><b className={entry.type}>{entry.type==="income"?"+ ":entry.type==="expense"?"− ":""}{budgetMoney(entry.amount)}</b><ChevronRight/></button>}):<div className="budget-empty"><WalletCards/><h3>Inga poster matchar</h3></div>}</section></>:<section className="budget-list budget-recurring-list"><header><div><small>AUTOMATISKA POSTER</small><h2>ÅTERKOMMANDE</h2></div><b>{entries.filter(entry=>entry.recurrence==="monthly").length} st</b></header>{entries.filter(entry=>entry.recurrence==="monthly").sort((a,b)=>a.date.localeCompare(b.date)).map(entry=><button key={entry.id} className="budget-row" onClick={()=>{if(entry.type==="transfer"){setEditingTransfer(entry);setTransferOpen(true)}else setEditing(entry)}}><span className={entry.type}>{entry.type==="income"?"+":entry.type==="expense"?"−":"⇄"}</span><span><strong>{entry.title}</strong><small>{entry.type==="transfer"?"Överföring":entry.category} · varje månad den {Number(entry.date.slice(-2))}:e · {entry.creatorName}</small></span><b className={entry.type}>{entry.type==="income"?"+ ":entry.type==="expense"?"− ":""}{budgetMoney(entry.amount)}</b><ChevronRight/></button>)}{!entries.some(entry=>entry.recurrence==="monthly")&&<div className="budget-empty"><History/><h3>Inga återkommande poster</h3><p>Markera “Återkommer varje månad” när du skapar en post.</p></div>}</section>}
    {selectedAccountId&&<BudgetAccountDetail accountId={selectedAccountId} settings={settings} entries={paidEntries} onClose={()=>setSelectedAccountId(undefined)}/>}
    ...
    {(creating||(editing!==undefined&&editing?.type!=="transfer"))&&<BudgetEditor entry={editing||undefined} initialType={createType} settings={settings} entries={monthEntries.map(item=>item.entry)} monthKey={monthKey} account={account} onClose={()=>{setEditing(undefined);onCreating(false)}} onSave={async entry=>{await onSave(entry);setEditing(undefined);onCreating(false)}} onDelete={editing?()=>setConfirmDelete(editing):undefined}/>} {settingsOpen&&<BudgetSettingsEditor settings={settings} onClose={()=>setSettingsOpen(false)} onSave={async value=>{await onSaveSettings(value);setSettingsOpen(false)}} onReset={async()=>{await onReset();setSettingsOpen(false)}} onClearMoney={async()=>{await onClearMoney();setSettingsOpen(false)}}/>}
...
function BudgetSettingsEditor({settings,onClose,onSave,onReset,onClearMoney}:{settings:BudgetSettings;onClose:()=>void;onSave:(settings:BudgetSettings)=>Promise<void>;onReset:()=>Promise<void>;onClearMoney:()=>Promise<void>}){
  const [banks,setBanks]=useState(settings.banks),[categoryBudgets,setCategoryBudgets]=useState(settings.categoryBudgets||{}),[goals,setGoals]=useState(settings.savingsGoals||[]),[resetStep,setResetStep]=useState(0),[resetText,setResetText]=useState(""),[moneyResetStep,setMoneyResetStep]=useState(0);
  ...
  return <div className="modal-backdrop"><section className="modal budget-settings-editor"><button className="modal-x" onClick={onClose}><X/></button><header><Settings/><h2>BUDGETINSTÄLLNINGAR</h2></header>
    <div className="budget-settings-explanation"><strong>Ingående saldo och avstämning</strong><p><b>Ingående saldo</b> är pengarna som fanns på kontot när du började använda Bubbget. <b>Stäm av saldo</b> är bankens verkliga saldo just nu. Om det skiljer sig från Bubbgets beräkning visas hur mycket högre eller lägre bankens saldo är.</p><small>Lämna “Stäm av saldo” tomt om du inte vill jämföra ännu.</small></div>
    <h3>BANKER, KONTON & SALDON</h3><div className="budget-bank-editor">{banks.map(bank=><section key={bank.id}><div><input value={bank.name} placeholder="Namn på bank" onChange={event=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,name:event.target.value}:item))}/><button onClick={()=>setBanks(old=>old.filter(item=>item.id!==bank.id))}><Trash2/></button></div>{bank.accounts.map(account=><div className="budget-account-edit budget-account-edit-full" key={account.id}><select aria-label="Kontoikon" value={account.icon||"wallet"} onChange={event=>updateAccount(bank.id,account.id,{icon:event.target.value})}>{budgetAccountIcons.map(item=><option value={item.id} key={item.id}>{item.icon} {item.label}</option>)}</select><input value={account.name} placeholder="Namn på konto" onChange={event=>updateAccount(bank.id,account.id,{name:event.target.value})}/><label>INGÅENDE SALDO<input inputMode="decimal" value={account.openingBalance||""} placeholder="0 kr" onChange={event=>updateAccount(bank.id,account.id,{openingBalance:Number(event.target.value.replace(",","."))||0})}/></label><label>STÄM AV SALDO<input inputMode="decimal" value={account.reconciledBalance??""} placeholder="Bankens saldo" onChange={event=>updateAccount(bank.id,account.id,{reconciledBalance:event.target.value===""?undefined:Number(event.target.value.replace(",","."))||0,reconciledAt:Date.now()})}/></label><button onClick={()=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,accounts:item.accounts.filter(value=>value.id!==account.id)}:item))}><Trash2/></button></div>)}<button className="add-account" onClick={()=>setBanks(old=>old.map(item=>item.id===bank.id?{...item,accounts:[...item.accounts,{id:crypto.randomUUID(),name:"",icon:"wallet",openingBalance:0}]}:item))}><Plus/> LÄGG TILL KONTO</button></section>)}</div><button className="add-bank" onClick={()=>setBanks(old=>[...old,{id:crypto.randomUUID(),name:"",accounts:[]}])}><Plus/> LÄGG TILL BANK</button>
    <section className="budget-settings-block"><h3>KATEGORIBUDGETAR</h3><div className="budget-settings-explanation"><strong>Så fungerar det</strong><p>Skriv hur mycket du högst vill spendera i varje kategori per månad. På översikten ser du hur mycket som använts och får en varning om gränsen passeras. Endast betalda utgifter räknas – planerade poster räknas först när de markeras som betalda.</p><small>Lämna ett fält tomt om kategorin inte ska ha någon gräns.</small></div><div className="budget-limit-grid">{budgetCategories.expense.map(category=><label key={category}><span>{budgetCategoryIcons[category]} {category}</span><span className="budget-limit-input"><input inputMode="decimal" value={categoryBudgets[category]||""} placeholder="Ingen gräns" aria-label={`${category}, kronor per månad`} onChange={event=>setCategoryBudgets(old=>({...old,[category]:Number(event.target.value.replace(",","."))||0}))}/><small>kr/mån</small></span></label>)}</div></section>
    <section className="budget-settings-block"><h3>SPARMÅL</h3>{goals.map(goal=><div className="budget-goal-edit" key={goal.id}><input value={goal.name} placeholder="Till exempel Semester" onChange={event=>setGoals(old=>old.map(item=>item.id===goal.id?{...item,name:event.target.value}:item))}/><input inputMode="decimal" value={goal.saved||""} placeholder="Sparat" onChange={event=>setGoals(old=>old.map(item=>item.id===goal.id?{...item,saved:Number(event.target.value)||0}:item))}/><input inputMode="decimal" value={goal.target||""} placeholder="Mål" onChange={event=>setGoals(old=>old.map(item=>item.id===goal.id?{...item,target:Number(event.target.value)||0}:item))}/><button onClick={()=>setGoals(old=>old.filter(item=>item.id!==goal.id))}><Trash2/></button></div>)}<button className="add-account" onClick={()=>setGoals(old=>[...old,{id:crypto.randomUUID(),name:"",saved:0,target:0}])}><Plus/> LÄGG TILL SPARMÅL</button></section>
  ...
  return (
    ...
    >
      <section className="modal budget-editor">
        ...
        <div className="budget-status-picker"><button type="button" className={status==="paid"?"selected":""} onClick={()=>setStatus("paid")}>✓ BETALD</button><button type="button" className={status==="planned"?"selected":""} onClick={()=>setStatus("planned")}>◷ PLANERAD</button></div>
...
function AuthenticatedApp() {
  ...
  const activeBudgetSettings=privateMode?privateBudgetSettings:budgetSettings;
  ...
  const persistBudgetEntry=async(entry:BudgetEntry)=>{if(!user||!account)return;const complete={...entry};if(privateMode)await savePrivateBudgetEntry(user.uid,complete);else if(account.activeGroupId)await saveBudgetEntry(account.activeGroupId,complete)};
  ...
  const persistBudgetSettings=async(settings:BudgetSettings)=>{if(!user||!account)return;if(privateMode)await savePrivateBudgetSettings(user.uid,settings);else if(account.activeGroupId)await saveBudgetSettings(account.activeGroupId,settings)};
