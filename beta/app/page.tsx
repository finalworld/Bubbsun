...
function RecipePrintButton(){
  const printRecipe=(event:React.MouseEvent<HTMLButtonElement>)=>{
    const source=event.currentTarget.closest<HTMLElement>(".recipe-view");
    if(!source)return;
    ...
    recipe.querySelectorAll(".recipe-close,.recipe-view-tools,footer,button").forEach(node=>node.remove());
