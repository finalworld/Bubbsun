...
function BudgetPage({entries,privateMode,account,memberships,groups,creating,onCreating,onMode,onSwitchGroup,onSave,onDelete}:{entries:BudgetEntry[];privateMode:boolean;account:Account;memberships:Membership[];groups:Record<string,Group>;creating:boolean;onCreating:(value:boolean)=>void;onMode:(value:boolean)=>void;onSwitchGroup:(id:string)=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete:(entry:BudgetEntry)=>Promise<void>}){
...
function BudgetEditor({entry,monthKey,account,onClose,onSave,onDelete}:{entry?:BudgetEntry;monthKey:string;account:Account;onClose:()=>void;onSave:(entry:BudgetEntry)=>Promise<void>;onDelete?:()=>void}){
  const [type,setType]=useState<"income"|"expense">(entry?.type||"expense"),[title,setTitle]=useState(entry?.title||""),[amount,setAmount]=useState(entry?String(entry.amount):""),[category,setCategory]=useState(entry?.category||"Mat"),[subcategory,setSubcategory]=useState(entry?.subcategory||"Hyra/avgift"),[date,setDate]=useState(entry?.date||`${monthKey}-01`),[recurring,setRecurring]=useState(entry?.recurrence==="monthly"),[note,setNote]=useState(entry?.note||""),[busy,setBusy]=useState(false);
...
function AuthenticatedApp() {
  ...
  return (
    <main className={`app-shell theme-${activeTheme.id}`} style={themeStyle}>
      ...
      {page === "budget" && <BudgetPage entries={activeBudgetEntries} privateMode={privateMode} account={account} memberships={memberships} groups={groups} creating={addingBudget} onCreating={setAddingBudget} onMode={setPrivateMode} onSwitchGroup={async id=>{await switchGroup(user.uid,id);setPrivateMode(false)}} onSave={persistBudgetEntry} onDelete={deleteBudgetEntry}/>}
