...
function SortableItemRow({
  ...
  return (
    ...
    {expanded&&!selecting&&<div id={`item-editor-${item.id}`} className={`item-inline-editor ${flashUnsaved?"flash-unsaved":""} ${listType==="links"?"saved-link-editor":""}`}>
      ...
      {listType==="links"&&<><label>Länk<input type="url" inputMode="url" value={draftLinkUrl} onChange={event=>{setDraftLinkUrl(event.target.value);onDirtyChange(true)}} placeholder="https://…" /></label><div className="saved-link-preview-column"><strong>Förhandsvisning</strong>{(draftLinkImage||cleanLinkUrl(draftLinkUrl))&&<img className="saved-link-edit-preview" src={draftLinkImage||automaticLinkImage(cleanLinkUrl(draftLinkUrl))} alt="Förhandsvisning"/>}<label className="saved-link-upload"><ImagePlus/> Byt bild<input type="file" accept="image/*" onChange={async event=>{const file=event.target.files?.[0];if(file){setDraftLinkImage(await compressLinkImage(file));onDirtyChange(true)}}}/></label></div></>}
...
function ListPage({
  ...
  const [linkCropZoom,setLinkCropZoom]=useState(1);
  const [linkCropX,setLinkCropX]=useState(0);
  const [linkCropY,setLinkCropY]=useState(0);
    ...
    <section className={`content detail-page ${list.listType==="links"?"links-detail-page":""}`}>
      ...
      <div className={`add-panel ${["wishlist","packing","orders"].includes(list.listType||"") ? "add-panel-stacked-select" : ""} ${list.listType==="cleaning" ? "add-panel-cleaning" : ""} ${list.listType==="home" ? "add-panel-homefix" : ""} ${list.listType==="links" ? "add-panel-links" : ""}`}>
        ...
        <div className={showNewItemNote?"has-add-note":""}>
          ...
          {list.listType==="links"&&<><div className="saved-link-image-choice"><span>{newLinkImage?"Sparad bild · klicka på bilden för att justera utsnittet":"Sidbild hämtad · klicka på bilden för att justera utsnittet"}</span><label className={linkImageBusy?"busy":""}><ImagePlus/>{linkImageBusy?"KOMPRIMERAR…":"VÄLJ EGEN BILD"}<input type="file" accept="image/*" disabled={linkImageBusy} onChange={async event=>{const file=event.target.files?.[0];if(!file)return;setLinkImageBusy(true);try{setNewLinkImage(await compressLinkImage(file));setLinkCropZoom(1);setLinkCropX(0);setLinkCropY(0)}finally{setLinkImageBusy(false)}}}/></label>{newLinkImage&&<button type="button" onClick={()=>setNewLinkImage("")}>ANVÄND AUTOMATISK</button>}</div>{(newLinkImage||cleanLinkUrl(newLinkUrl))?<button type="button" className="saved-link-preview-button editable" disabled={linkImageBusy} onClick={async()=>{if(newLinkImage){setLinkCropOpen(true);return}const pageUrl=cleanLinkUrl(newLinkUrl);if(!pageUrl)return;setLinkImageBusy(true);try{setNewLinkImage(await importLinkImage(automaticLinkImage(pageUrl)));setLinkCropZoom(1);setLinkCropX(0);setLinkCropY(0);setLinkCropOpen(true)}catch{window.alert("Sidbilden kunde inte öppnas för redigering. Du kan fortfarande välja en egen bild.")}finally{setLinkImageBusy(false)}}} aria-label="Justera bildens utsnitt"><img className="saved-link-new-preview" src={newLinkImage||automaticLinkImage(cleanLinkUrl(newLinkUrl))} alt="Förhandsvisning av sidan"/></button>:<div className="saved-link-empty-preview"><ImagePlus/><span>INGEN BILD ÄN</span></div>}</>}
      ...
      {linkCropOpen&&newLinkImage&&<div className="modal-backdrop link-crop-backdrop"><div className="link-crop-modal"><h2>JUSTERA BILDEN</h2><div className="link-crop-viewport"><img src={newLinkImage} alt="Bildutsnitt" style={{transform:`translate(${linkCropX*.45}%,${linkCropY*.45}%) scale(${linkCropZoom})`}}/></div><label>ZOOM<input type="range" min="1" max="3" step="0.05" value={linkCropZoom} onChange={event=>setLinkCropZoom(Number(event.target.value))}/></label><label>FLYTTA SIDLEDS<input type="range" min="-100" max="100" value={linkCropX} onChange={event=>setLinkCropX(Number(event.target.value))}/></label><label>FLYTTA UPP/NED<input type="range" min="-100" max="100" value={linkCropY} onChange={event=>setLinkCropY(Number(event.target.value))}/></label><div><button className="cancel" onClick={()=>setLinkCropOpen(false)}>AVBRYT</button><button onClick={async()=>{setLinkImageBusy(true);try{setNewLinkImage(await cropLinkImage(newLinkImage,linkCropZoom,linkCropX,linkCropY));setLinkCropOpen(false);setLinkCropZoom(1);setLinkCropX(0);setLinkCropY(0)}finally{setLinkImageBusy(false)}}}>ANVÄND UTSNITT</button></div></div></div>}
      {selecting && (
        <div
