...
function LinkImageCropModal({source,onCancel,onApply}:{source:string;onCancel:()=>void;onApply:(image:string)=>void}){const [zoom,setZoom]=useState(1),[x,setX]=useState(0),[y,setY]=useState(0),[busy,setBusy]=useState(false),drag=useRef<{x:number;y:number;pointerId:number}|null>(null);const clamp=(value:number)=>Math.max(-100,Math.min(100,value)),stopDrag=()=>{drag.current=null},travel=(zoom-1)/(zoom*2);if(typeof document==="undefined")return null;return createPortal(<div className="modal-backdrop link-crop-backdrop"><div className="link-crop-modal"><h2>JUSTERA BILDEN</h2><div className="link-crop-viewport" onDragStart={event=>event.preventDefault()} onPointerDown={event=>{event.preventDefault();drag.current={x:event.clientX,y:event.clientY,pointerId:event.pointerId};event.currentTarget.setPointerCapture(event.pointerId)}} onPointerMove={event=>{const start=drag.current;if(!start||start.pointerId!==event.pointerId)return;event.preventDefault();const box=event.currentTarget.getBoundingClientRect(),dx=event.clientX-start.x,dy=event.clientY-start.y;start.x=event.clientX;start.y=event.clientY;setX(value=>clamp(value+(dx/box.width)*140/zoom));setY(value=>clamp(value+(dy/box.height)*140/zoom))}} onPointerUp={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);stopDrag()}} onPointerCancel={stopDrag} onLostPointerCapture={stopDrag}><img draggable={false} src={source} alt="Bildutsnitt" style={{transform:`scale(${zoom}) translate(${x*travel}%,${y*travel}%)`}}/></div><p className="link-crop-hint">Ta tag i bilden och dra den till rätt läge.</p><label>ZOOM<input type="range" min="1" max="3" step="0.05" value={zoom} onChange={event=>setZoom(Number(event.target.value))}/></label><div><button className="cancel" disabled={busy} onClick={onCancel}>AVBRYT</button><button disabled={busy} onClick={async()=>{setBusy(true);try{onApply(await cropLinkImage(source,zoom,x,y))}finally{setBusy(false)}}}>{busy?"SPARAR…":"ANVÄND UTSNITT"}</button></div></div></div>,document.body)}
