...
export async function saveRecipe(groupId:string,recipe:Recipe){const sourcePath=`groups/${groupId}/recipes/${recipe.id}`,stored={...recipe,sourcePath};await setDoc(doc(db,sourcePath),stored,{merge:true});await syncRecipePublication(sourcePath,stored,"group",groupId);}
export async function savePrivateRecipe(uid:string,recipe:Recipe){const sourcePath=`users/${uid}/privateRecipes/${recipe.id}`,stored={...recipe,sourcePath};await setDoc(doc(db,sourcePath),stored,{merge:true});await syncRecipePublication(sourcePath,stored,"private");}
export async function moveRecipeLocation(uid:string,recipe:Recipe,fromLocation:string,toLocation:string){
  ...
  const stored={...recipe,sourcePath:toPath};
