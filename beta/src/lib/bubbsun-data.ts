...
const budgetEntryForStorage=(entry:BudgetEntry)=>JSON.parse(JSON.stringify({...entry,subcategory:entry.subcategory||"",recurrence:entry.recurrence||"",status:entry.status||"paid",paidAt:entry.paidAt||0,note:entry.note||""}));
...
export async function clearBudgetMoney(groupId:string,settings:BudgetSettings){const snap=await getDocs(collection(db,"groups",groupId,"budgetEntries")),batch=writeBatch(db),clean={...settings,banks:settings.banks.map(bank=>({...bank,accounts:bank.accounts.map(account=>({...account,openingBalance:0,reconciledBalance:0,reconciledAt:Date.now()}))})),updatedAt:Date.now()};snap.docs.forEach(item=>batch.delete(item.ref));batch.set(doc(db,"groups",groupId,"budgetSettings","config"),JSON.parse(JSON.stringify(clean)));await batch.commit();}
