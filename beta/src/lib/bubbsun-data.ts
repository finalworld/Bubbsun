...
const parseRecipe=(item:{id:string;data:()=>Record<string,unknown>;ref?:{path:string}}):Recipe=>{const d=item.data();return{id:item.id,title:textValue(d.title,"Namnlöst recept"),category:textValue(d.category),subcategory:textValue(d.subcategory),isPublic:d.isPublic===true,sourcePath:item.ref?.path,image:textValue(d.image),servings:Math.max(1,numberValue(d.servings,4)),minutes:Math.max(0,numberValue(d.minutes)),ingredients:Array.isArray(d.ingredients)?d.ingredients.map((raw,index)=>{const value=(raw&&typeof raw==="object"?raw:{}) as Record<string,unknown>;return{id:textValue(value.id,String(index)),amount:textValue(value.amount),unit:textValue(value.unit),name:textValue(value.name)}}).filter(value=>value.name):[],instructions:textValue(d.instructions),note:textValue(d.note),linkedListId:textValue(d.linkedListId),creatorId:textValue(d.creatorId),creatorName:textValue(d.creatorName,"Bubbsun"),creatorColor:numberValue(d.creatorColor,0xff2b7a78),createdAt:numberValue(d.createdAt),updatedAt:numberValue(d.updatedAt),updatedBy:textValue(d.updatedBy)}};
...
export function watchPublicRecipes(callback:(recipes:Recipe[])=>void):Unsubscribe{let groupRecipes:Recipe[]=[],privateRecipes:Recipe[]=[];const emit=()=>callback([...groupRecipes,...privateRecipes].sort((a,b)=>b.updatedAt-a.updatedAt));const a=onSnapshot(query(collectionGroup(db,"recipes"),where("isPublic","==",true)),snap=>{groupRecipes=snap.docs.map(parseRecipe);emit()});const b=onSnapshot(query(collectionGroup(db,"privateRecipes"),where("isPublic","==",true)),snap=>{privateRecipes=snap.docs.map(parseRecipe);emit()});return()=>{a();b()};}
export async function saveRecipe(groupId:string,recipe:Recipe){await setDoc(doc(db,"groups",groupId,"recipes",recipe.id),recipe,{merge:true});}
export async function savePrivateRecipe(uid:string,recipe:Recipe){await setDoc(doc(db,"users",uid,"privateRecipes",recipe.id),recipe,{merge:true});}
export async function removeRecipe(groupId:string,id:string){await deleteDoc(doc(db,"groups",groupId,"recipes",id));}
export async function removePrivateRecipe(uid:string,id:string){await deleteDoc(doc(db,"users",uid,"privateRecipes",id));}
export async function unpublishRecipe(sourcePath:string){await updateDoc(doc(db,sourcePath),{isPublic:false,updatedAt:Date.now()});}
