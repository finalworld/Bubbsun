...
const budgetEntryForStorage=(entry:BudgetEntry)=>JSON.parse(JSON.stringify({...entry,subcategory:entry.subcategory||"",recurrence:entry.recurrence||"",status:entry.status||"paid",autoPay:entry.autoPay===true,paidAt:entry.paidAt||0,note:entry.note||""}));
