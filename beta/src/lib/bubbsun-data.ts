...
const parseRecipe=(item:{id:string;data:()=>Record<string,unknown>}):Recipe=>{const d=item.data();return{id:item.id,title:textValue(d.title,"Namnlöst recept"),category:textValue(d.category),image:textValue(d.image),servings:Math.max(1,numberValue(d.servings,4)),minutes:Math.max(0,numberValue(d.minutes)),ingredients:Array.isArray(d.ingredients)?d.ingredients.map((raw,index)=>{const value=(raw&&typeof raw==="object"?raw:{}) as Record<string,unknown>;return{id:textValue(value.id,String(index)),amount:textValue(value.amount),unit:textValue(value.unit),name:textValue(value.name)}}).filter(value=>value.name):[],instructions:textValue(d.instructions),note:textValue(d.note),linkedListId:textValue(d.linkedListId),creatorId:textValue(d.creatorId),creatorName:textValue(d.creatorName,"Bubbsun"),creatorColor:numberValue(d.creatorColor,0xff2b7a78),createdAt:numberValue(d.createdAt),updatedAt:numberValue(d.updatedAt),updatedBy:textValue(d.updatedBy)}};
