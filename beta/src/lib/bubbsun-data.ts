...
import type { Account, AdminUserCounts, BudgetEntry, BubbsunList, BubbsunNote, CalendarEvent, DirectChat, DirectMessage, GlobalPin, Group, JoinRequest, ListItem, Membership, PublicListShare, Recipe, Report, ThemePalette } from "../types";
...
const parseBudgetEntry=(item:{id:string;data:()=>Record<string,unknown>}):BudgetEntry=>{const d=item.data();return{id:item.id,type:d.type==="income"?"income":"expense",title:textValue(d.title,"Budgetpost"),amount:Math.max(0,numberValue(d.amount)),category:textValue(d.category,"Övrigt"),date:textValue(d.date),note:textValue(d.note),creatorId:textValue(d.creatorId),creatorName:textValue(d.creatorName,"Bubbsun"),createdAt:numberValue(d.createdAt),updatedAt:numberValue(d.updatedAt)}};
...
export async function saveBudgetEntry(groupId:string,entry:BudgetEntry){await setDoc(doc(db,"groups",groupId,"budgetEntries",entry.id),entry,{merge:true});}
export async function savePrivateBudgetEntry(uid:string,entry:BudgetEntry){await setDoc(doc(db,"users",uid,"privateBudgetEntries",entry.id),entry,{merge:true});}
