...
const parseCalendarEvent=(item:{id:string;data:()=>Record<string,unknown>}):CalendarEvent=>{const d=item.data();return{id:item.id,title:textValue(d.title),date:textValue(d.date),time:textValue(d.time),endTime:textValue(d.endTime),allDay:d.allDay===true,category:textValue(d.category,"other"),color:numberValue(d.color),recurrenceDays:Array.isArray(d.recurrenceDays)?d.recurrenceDays.filter((value):value is number=>typeof value==="number"):[],recurrenceForever:d.recurrenceForever===true,recurrenceUntil:textValue(d.recurrenceUntil),excludedDates:Array.isArray(d.excludedDates)?d.excludedDates.filter((value):value is string=>typeof value==="string"):[],note:textValue(d.note),linkedListIds:Array.isArray(d.linkedListIds)?d.linkedListIds.filter((value):value is string=>typeof value==="string").slice(0,3):[],reminderMinutes:numberValue(d.reminderMinutes),creatorId:textValue(d.creatorId),creatorName:textValue(d.creatorName,"Bubbsun"),createdAt:numberValue(d.createdAt),updatedAt:numberValue(d.updatedAt)}};
