...
import { DiceBoard } from "./dice-view.mjs?v=mobile-rack57";
...
const dice = Array.from({ length: 5 }, (_, id) => ({ id, value: id + 1, held: false, x:0, y:0, rot:0,
  pose:{position:[[-1.5,1.2,-.1,-1.5,1.2][id],.5,[-1.5,-1.5,0,1.5,1.5][id]],quaternion:faceQuaternion(id+1,(id-2)*.12)} }));
...
let rolls = 0, busy = false, playerScores = {}, aiScores = {}, lastScore = null, profile = loadProfile(), remoteMatch = null, suggestionTimer = 0, dicePhysicsPromise = Promise.resolve(), physicsBusy = false, diceBoard = null;
function profileKey() { return `yatsun-profile-${authUser?.uid||"guest"}`; }
function loadProfile() { const base={soloXp:0,unlocked:1,winStreak:0,bestWinStreak:0,matchesPlayed:0,wins:0,draws:0,losses:0,yatzyCount:0,totalScore:0,bestScore:0};try { return {...base,...JSON.parse(localStorage.getItem(profileKey())||"{}")}; } catch { return base; } }
...
function syncCosmetics(selection=false){
  const account=authUser;if(!account)return Promise.resolve();const data={action:'social_cosmetics',completed:completedLevels(profile),stats:{soloXp:profile.soloXp||0,matches:profile.matchesPlayed||0,wins:profile.wins||0,draws:profile.draws||0,losses:profile.losses||0,yatzy:profile.yatzyCount||0,totalScore:profile.totalScore||0,bestScore:profile.bestScore||0,bestStreak:profile.bestWinStreak||0}};if(selection)data.active=activeSkin(profile).id;
  const request=cosmeticQueue.catch(()=>{}).then(async()=>{const token=await account.getIdToken(),response=await fetch('./api/',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify(data)}),result=await response.json();if(!response.ok)throw new Error(result.error||'Kunde inte spara tärningsvalet.');if(authUser?.uid!==account.uid)return;const saved=result.stats||{},serverStats={soloXp:saved.solo_xp,matchesPlayed:saved.matches,wins:saved.wins,draws:saved.draws,losses:saved.losses,yatzyCount:saved.yatzy,totalScore:saved.total_score,bestScore:saved.best_score,bestWinStreak:saved.best_streak};for(const [key,value] of Object.entries(serverStats))profile[key]=Math.max(Number(profile[key])||0,Number(value)||0);profile.completed=Math.max(completedLevels(profile),result.completed);profile.unlocked=Math.min(100,profile.completed+1);profile.activeSkin=result.active;profile.admin=Boolean(result.admin);localStorage.setItem(profileKey(),JSON.stringify(profile));updateProfileUi();});cosmeticQueue=request;return request;
}
...
function saveMatch(){if(physicsBusy||online?.active||matchComplete())return;const state={rolls,playerScores,aiScores,lastScore,dice:dice.map(({value,held,x,y,rot,q,pose})=>({value,held,x,y,rot,q,pose})),opponentLevel:profile.unlocked,updatedAt:Date.now()};remoteMatch=state;localStorage.setItem(matchKey(),JSON.stringify(state));updateContinueUi();void matchApi("save_match",state).catch((error)=>console.warn("Matchen sparades lokalt men inte på servern",error));}
...
function opponentBio(level=profile.unlocked){
  if(level===1)return 'Maj-Britt är 87 år och nattvandrar helst i Orsa. De nya skorna köptes bara för detta, men i ena fickan ligger alltid fem tärningar. Hon har fler års Yatsun-erfarenhet än de flesta har kvitton i kökslådan. Vågar du möta henne?';
  const ages=[42,68,31,76,55,24,83,47,62,39,71,29,58,64,36,79,51,27,73,45],hobbies=['samlar på borttappade vantar','tränar agility med sin dammsugare','odlar tävlingsgurkor','bygger fågelholkar med balkong','recenserar kommunala rondeller','dansar foxtrot i tvättstugan','letar efter Sveriges rakaste pinne','stickar mössor åt brevlådor','fotograferar ovanligt självsäkra måsar','tävlingssorterar smågodis'],places=['Arvidsjaur','Kumla','Sveg','Mellerud','Trosa','Falkenberg','Kiruna','Nora','Tranås','Ystad'],purchases=['en visselpipa av mahogny','sju identiska termosar','en regnrock för sin cykel','ett förvånansvärt tungt förstoringsglas','ett campingbord med inbyggd osthyvel','en hatt som bara används på tisdagar','en ficklampa med discokula','en elektrisk äggtimer','ett par tävlingssockor','en portabel fanfar'],habits=['slår tärningarna med vänster hand för att högern är reserverad för segergester','vägrar kasta innan alla i rummet har sagt ”lycka till”','påstår sig kunna höra skillnad på en fyra och en femma','för protokoll över varje etta sedan 1998','värmer upp med tre minuters intensiv ögonkontakt med tärningarna','har döpt varje tärning efter en svensk insjö','kastar bara när kaffet står exakt norr om blocket','firar varje kåk med en mycket liten trudelutt','anser att stegar blir bättre om man sitter på en kudde','har ett hemligt system som mest verkar bestå av att nynna'],endings=['Det låter harmlöst, men poängblocket säger något annat.','Ingen vet om metoden fungerar, men ingen vågar fråga.','Underskatta inte lugnet – där bakom räknas det snabbt.','Kan du stoppa segersviten innan fanfaren åker fram?','Ryktet säger att en Yatzy aldrig glöms och sällan förlåts.'];
  const i=level-1,name=opponentName(level);return `${name} är ${ages[i%ages.length]} år, bor i ${places[(i*3)%places.length]} och ${hobbies[(i*7)%hobbies.length]}. Nyligen köptes ${purchases[(i*9)%purchases.length]} av skäl som ingen riktigt förstått. Vid Yatsunbordet ${habits[(i*11)%habits.length]}. ${endings[(i*13)%endings.length]}`;
}
function updateContinueUi(){const saved=loadMatch(),button=$("#start-solo");if(!button)return;const level=Math.min(100,Math.max(1,Number(saved?.opponentLevel||profile.unlocked)||1)),name=opponentName(level);button.textContent=saved?`FORTSÄTT MOT ${name.toLocaleUpperCase("sv-SE")} →`:`SPELA MOT ${name.toLocaleUpperCase("sv-SE")} →`;$("#solo-opponent-level").textContent=`SINGELPLAYER · NIVÅ ${level} AV 100`;$("#solo-opponent-name").textContent=name;$("#solo-opponent-bio").textContent=opponentBio(level);}
function updateProfileUi() { const {level,xp,needed}=playerProgress(); $("#home-player-name").textContent=authDisplayName; $("#home-player-progress").textContent=`Nivå ${level} · ${xp}/${needed} XP`; $(".home-xp-meter em").style.width=`${xp/needed*100}%`; updateContinueUi(); }
function showScreen(id) { ["#mode-screen","#lobby-screen","#game-screen","#leaderboard-screen","#campaign-screen","#collection-screen","#mp-home-screen","#mp-join-screen","#profile-stats-screen"].forEach((selector)=>$(selector)?.classList.toggle("hidden",selector!==id));$('.welcome-strip')?.classList.add('hidden'); document.body.classList.toggle("playing",id==="#game-screen");document.body.dataset.screen=id.slice(1);if(id==='#game-screen'&&!online?.active)updateMatchPlayers(false); window.scrollTo({top:0,behavior:"smooth"}); }

const profileStatsScreen=document.createElement('section');profileStatsScreen.id='profile-stats-screen';profileStatsScreen.className='profile-stats-page hidden';document.querySelector('main').append(profileStatsScreen);
function openPlayerProfile(person=null){const mine=!person,stats=mine?{...profile,name:authDisplayName,completed:completedLevels(profile),active:activeSkin(profile).name}:{...person,name:person.name||'Yatsunspelare'},xp=Number(stats.soloXp||stats.solo_xp)||0,progress=mine?playerProgress(xp):playerProgress(xp),played=Number(stats.matchesPlayed??stats.matches??0),wins=Number(stats.wins||0),draws=Number(stats.draws||0),losses=Number(stats.losses||0),best=Number(stats.bestScore??stats.best_score??0),total=Number(stats.totalScore??stats.total_score??0),yatzy=Number(stats.yatzyCount??stats.yatzy??0),streak=Number(stats.bestWinStreak??stats.best_streak??0),completed=Number(stats.completed||0),rate=played?Math.round(wins/played*100):0,average=played?Math.round(total/played):0;profileStatsScreen.innerHTML=`<header class="profile-stats-hero"><span class="profile-stats-avatar">${mine?'DU':String(stats.name).slice(0,1).toLocaleUpperCase('sv-SE')}</span><div><small>${mine?'DIN SPELARPROFIL':'VÄNPROFIL'}</small><h1>${stats.name}</h1><p>Nivå ${progress.level} · ${progress.xp}/${progress.needed} XP</p><i><em style="width:${Math.min(100,progress.xp/progress.needed*100)}%"></em></i></div></header><div class="profile-stat-grid"><article><span>Matcher</span><b>${played}</b></article><article><span>Vinster</span><b>${wins}</b></article><article><span>Vinstprocent</span><b>${rate}%</b></article><article><span>Bästa poäng</span><b>${best}</b></article><article><span>Snittpoäng</span><b>${average}</b></article><article><span>Yatzy</span><b>${yatzy}</b></article><article><span>Bästa svit</span><b>${streak}</b></article><article><span>Oavgjorda / förluster</span><b>${draws} / ${losses}</b></article></div><section class="profile-progress-card"><div><small>ÄVENTYRET</small><h2>${completed} av 100 besegrade</h2><p>${Math.max(1,Math.ceil((completed+1)/10))} tärningsset upplåsta</p></div><strong>${Math.min(100,completed)}%</strong></section>`;showScreen('#profile-stats-screen');}
$('#home-profile-button').addEventListener('click',()=>openPlayerProfile());
...
function renderDice(animate=false) {
  if(physicsBusy)return dicePhysicsPromise;
  dice.forEach(die=>{die.skin=online?.active?boardSkin:activeSkin(profile).id;});
  if(!diceBoard)diceBoard=new DiceBoard(diceRoot,rollButton,(id)=>{
    if(!rolls||busy||physicsBusy)return;
    const die=dice.find(d=>d.id===id);die.held=!die.held;
    syncHeldDiceUi();updateSuggestions();
    if(online?.active)void online.hold(dice.map(d=>d.held));else saveMatch();
    rollHint.textContent=die.held?"Tärningen är sparad.":"Tärningen kastas igen nästa gång.";
  });
  clearTimeout(suggestionTimer);
  if(!animate){
    const ready=diceBoard.setDice(dice);
    ready.catch(error=>{console.error(error);rollHint.textContent="Tärningarna kunde inte laddas. Ladda om sidan.";});
    updateSuggestions();return ready;
  }
  physicsBusy=true;hideSuggestions();setRollButtonState();diceRoot.classList.add("physics-rolling");
  dicePhysicsPromise=diceBoard.roll(dice,online?.active?dice.map(d=>d.value):null).then(()=>{
    physicsBusy=false;
    if(!online?.active&&!busy)saveMatch();
    return true;
  }).catch(error=>{
    physicsBusy=false;console.error("Tärningskastet avbröts",error);
    rollHint.textContent=error.message;return false;
  }).finally(()=>{
    diceRoot.classList.remove("physics-rolling");
    updateSuggestions();
  });
  return dicePhysicsPromise;
}
function syncHeldDiceUi(){diceBoard?.syncHeld(dice);}
...
function updateSuggestions(){
  ...
  categories.forEach((category)=>{
    ...
    else{const preview=rolls&&!busy&&!physicsBusy?scoreCategory(category.id,values):null;button.textContent=preview===null||preview===0?"—":String(preview);button.disabled=!rolls||busy||physicsBusy;if(preview!==null){button.title=`Välj ${category.label}: ${preview} poäng`;row.classList.add("preview");if(preview===0)row.classList.add("zero-preview");previews.push({row,score:preview});}}
...
function startGame(){const saved=loadMatch();playerScores=saved?.playerScores||{};aiScores=saved?.aiScores||{};lastScore=saved?.lastScore||null;rolls=saved?.rolls||0;busy=false;restoreDice(saved?.dice);buildScorecard();if(!saved)resetTurn();else{renderDice();rollNumber.textContent=String(Math.max(1,rolls));rollButton.disabled=rolls>=3;rollButton.querySelector("small").textContent=`${3-rolls} kast kvar`;rollHint.textContent=rolls?"Fortsätt där du slutade.":"Kasta alla fem tärningarna.";}const level=saved?.opponentLevel||profile.unlocked,name=opponentName(level),progress=playerProgress(),shortName=name.split(" ")[0];$$(".player-card")[0].querySelector("strong").textContent=authDisplayName;$$(".player-card")[0].querySelector("span").textContent=`Spelarnivå ${progress.level} · ${progress.xp}/${progress.needed} XP`;$(".score-panel h2").textContent=authDisplayName;$$('.ai-name').forEach((cell)=>cell.textContent=shortName);$$('.score-section-block>strong').forEach(label=>{if(label?.dataset)label.dataset.opponent=shortName;});$$(".player-card")[1].querySelector("strong").textContent=name;$$(".player-card")[1].querySelector("small").textContent="";$$(".player-card")[1].querySelector("span").textContent=`Nivå: ${level}/100`;$("#result-modal").classList.add("hidden");$(".turn-heading h2").textContent="Din tur!";showScreen("#game-screen");updateSuggestions();saveMatch();if(saved&&scoreCount(aiScores)<scoreCount(playerScores)){busy=true;void catchUpAiScores().then(()=>{if(matchComplete())finishMatch();else{busy=false;resetTurn();saveMatch();}}).catch((error)=>{busy=false;console.warn("Kunde inte återuppta motståndarens tur",error);});}}
function finishMatch(){
  ...
  profile.matchesPlayed=(profile.matchesPlayed||0)+1;profile.wins=(profile.wins||0)+(won?1:0);profile.draws=(profile.draws||0)+(draw?1:0);profile.losses=(profile.losses||0)+(!won&&!draw?1:0);profile.yatzyCount=(profile.yatzyCount||0)+(playerScores.l8===50?1:0);profile.totalScore=(profile.totalScore||0)+player;profile.bestWinStreak=Math.max(profile.bestWinStreak||0,profile.winStreak||0);
...
function updateMatchPlayers(multiplayer,room=null){
  ...
  cards[0].querySelector('small').textContent=multiplayer?(done?'MATCHEN KLAR':mine?'DIN TUR':'DU'):'';
  ...
  $$('.score-section-block>strong').forEach(label=>{if(label?.dataset)label.dataset.opponent=(multiplayer?(room.otherName||'Vän'):cards[1].querySelector('strong').textContent).split(' ')[0];});
...
function applyOnlineRoom(room,initial=false,sending=false,force=false){
  ...
  if(initial){showScreen('#game-screen');buildScorecard();dice.forEach((d,i)=>{d.x=(i%2?1:-1)*100;d.y=(Math.floor(i/2)-1)*145;d.rot=(i-2)*7;delete d.q;delete d.pose;});}
  if(changed){const previous=lastScore;playerScores=state.scores[uid];aiScores=state.scores[other];rolls=state.rolls;lastScore=state.last?{who:state.last.uid===uid?'player':'ai',id:state.last.id}:null;dice.forEach((d,i)=>{if(initial||(!rolled&&d.value!==state.dice[i])){delete d.q;delete d.pose;}d.value=state.dice[i];d.held=state.held[i];});if(rolled){onlineAnimating=true;playDiceSound();}renderDice(rolled);if(!initial&&state.last?.id==='l8'&&state.last.score===50&&(previous?.id!==lastScore.id||previous?.who!==lastScore.who))triggerYatsun();onlineRevision=room.revision;onlineSnapshot={id:room.id,turn:state.turn,rolls:state.rolls};}
...
online=createSocial({show:showScreen,showProfile:openPlayerProfile,user:()=>authUser,name:()=>authDisplayName,canEnter:()=>!onlineAnimating&&(online.active||(!busy&&!physicsBusy)),applyRoom:applyOnlineRoom,login:()=>$('#login-modal').classList.remove('hidden'),leaveRoom:()=>{onlineRevision=-1;onlineSnapshot=null;queuedOnlineRoom=null;busy=false;rollButton.classList.remove('hidden');$('#restart-match').hidden=false;$('#end-match').textContent='Avsluta match';showScreen('#mode-screen');}});
...
const extras=document.createElement('div');extras.className='solo-extras';const mapButton=document.createElement('button');mapButton.type='button';mapButton.className='solo-extra map';mapButton.innerHTML='<span class="solo-extra-icon" aria-hidden="true"><i></i><i></i><i></i></span><b>Öppna äventyrskartan</b><em>Se vägen genom alla 100 motståndare</em><strong>→</strong>';mapButton.setAttribute('aria-label','Öppna äventyrskartan');mapButton.onclick=journey.openMap;extras.append(mapButton);$('.solo-card').append(extras);
