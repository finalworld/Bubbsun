...
function updateSuggestions(){
  ...
  categories.forEach((category)=>{
    ...
    else{const preview=rolls&&!busy?scoreCategory(category.id,values):null;button.textContent=preview===null||preview===0?"—":String(preview);button.disabled=!rolls||busy;if(preview!==null){button.title=`Välj ${category.label}: ${preview} poäng`;row.classList.add("preview");if(preview===0)row.classList.add("zero-preview");previews.push({row,score:preview});}}
