...
function saveMatch(){if(online?.active||Object.keys(playerScores).length>=categories.length)return;const state={rolls,playerScores,aiScores,lastScore,dice:dice.map(({value,held,x,y,rot,q})=>({value,held,x,y,rot,q})),opponentLevel:profile.unlocked,updatedAt:Date.now()};remoteMatch=state;localStorage.setItem(matchKey(),JSON.stringify(state));updateContinueUi();void matchApi("save_match",state).catch((error)=>console.warn("Matchen sparades lokalt men inte på servern",error));}
...
function runDicePhysics(entries,width,height,authoritativeValues=null){
  ...
  dice.forEach((die,index)=>{
    const active=activeIds.has(die.id),handOffsets=[[0,-1.05],[-.92,-.53],[0,0],[-.92,.53],[0,1.05]],startX=active?halfWidth+handOffsets[index][0]:die.x/pixelsPerUnit,startZ=active?handOffsets[index][1]:die.y/pixelsPerUnit,body=new CANNON.Body({mass:active?1.15:0,material:diceMaterial,position:new CANNON.Vec3(startX,active?1.05+index*.08:half,startZ),shape:new CANNON.Box(new CANNON.Vec3(half,active?half:2,half)),linearDamping:.13,angularDamping:.27,allowSleep:true,sleepSpeedLimit:.09,sleepTimeLimit:.7});
  ...
  const faceNormals=[[3,0,0,1],[4,0,0,-1],[2,1,0,0],[5,-1,0,0],[6,0,1,0],[1,0,-1,0]],up=new CANNON.Vec3(0,1,0),alignment=(body)=>Math.max(...faceNormals.map(([,x,y,z])=>body.quaternion.vmult(new CANNON.Vec3(x,y,z)).y)),start=performance.now(),fixedStep=1/60,normalDuration=4600,hardLimit=6500;
  function frame(now){
