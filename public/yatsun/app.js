...
function runDicePhysics(entries,width,height){
  ...
  dice.forEach((die,index)=>{
    const active=activeIds.has(die.id),handOffsets=[[0,-1.05],[-.92,-.53],[0,0],[-.92,.53],[0,1.05]],startX=active?halfWidth+handOffsets[index][0]:die.x/pixelsPerUnit,startZ=active?handOffsets[index][1]:die.y/pixelsPerUnit,body=new CANNON.Body({mass:active?1.15:0,material:diceMaterial,position:new CANNON.Vec3(startX,active?1.05+index*.08:half,startZ),shape:new CANNON.Box(new CANNON.Vec3(half,active?half:2,half)),linearDamping:.07,angularDamping:.14,allowSleep:true,sleepSpeedLimit:.18,sleepTimeLimit:.5});
