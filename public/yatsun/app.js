...
function scorecardComplete(scores){return categories.every(({id})=>id in scores);}
...
async function chooseScore(id){if(!rolls||busy||id in playerScores)return;if(online?.active){await online.score(id);return;}const chosen=scoreCategory(id,dice.map((d)=>d.value));playerScores[id]=chosen;lastScore={who:"player",id};if(id==="l8"&&chosen===50)triggerYatsun();busy=true;updateSuggestions();saveMatch();await aiTurn();while(scorecardComplete(playerScores)&&!scorecardComplete(aiScores))await aiTurn();if(matchComplete()){finishMatch();return;}busy=false;resetTurn();saveMatch();}
