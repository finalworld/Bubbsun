...
function runDicePhysics(entries,width,height){
  ...
  function frame(now){
    ...
    if(now-start>1050)bodies.forEach((body)=>{
      ...
      if(body.position.y>half+.1){const neighbour=bodies.filter((other)=>other!==body&&other.mass>0).sort((a,b)=>Math.hypot(body.position.x-a.position.x,body.position.z-a.position.z)-Math.hypot(body.position.x-b.position.x,body.position.z-b.position.z))[0],dx=body.position.x-(neighbour?.position.x||0),dz=body.position.z-(neighbour?.position.z||0),length=Math.hypot(dx,dz)||1;body.lastEdgeTip=now;body.wakeUp();body.velocity.set(dx/length*.85,.28,dz/length*.85);body.angularVelocity.set(dz/length*1.05,.08,-dx/length*1.05);return;}
