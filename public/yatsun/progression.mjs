...
export function paintDice(model,skin,THREE){
  ...
  model.traverse(node=>{if(!node.isMesh)return;const tint=original=>{const material=original.clone();material.metalness=skin.metalness;material.roughness=skin.roughness;material.onBeforeCompile=shader=>{shader.uniforms.skinPips={value:new THREE.Color(skin.pips)};shader.uniforms.skinTexture={value:atlas};shader.uniforms.skinAtlas={value:new THREE.Vector4(x/1536,y/1024,w/1536,h/1024)};shader.vertexShader='varying vec3 skinObjectPosition;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nskinObjectPosition=position;');shader.fragmentShader='uniform vec3 skinPips; uniform sampler2D skinTexture; uniform vec4 skinAtlas; varying vec3 skinObjectPosition;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>','#include <map_fragment>\nfloat faceInk=smoothstep(0.12,0.65,dot(diffuseColor.rgb,vec3(0.2126,0.7152,0.0722))); vec3 p=skinObjectPosition,ap=abs(p); vec2 cubeUv=ap.x>ap.y&&ap.x>ap.z?p.zy:ap.y>ap.z?p.xz:p.xy; vec2 materialUv=clamp((cubeUv/.75)+.5,0.0,1.0); materialUv=mix(vec2(.22),vec2(.78),materialUv); vec3 surface=texture2D(skinTexture,skinAtlas.xy+materialUv*skinAtlas.zw).rgb; diffuseColor.rgb=mix(skinPips,surface,faceInk);');};material.customProgramCacheKey=()=>skin.id+'-material-core1';materials.push(material);return material;};node.material=Array.isArray(node.material)?node.material.map(tint):tint(node.material);});
  return materials;
}
