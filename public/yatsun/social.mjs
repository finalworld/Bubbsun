export function createSocial({user, name, applyRoom, leaveRoom, login, canEnter=()=>true,show=()=>{},showProfile=()=>{}}) {
  ...
  function pageHeading(root,label){root.replaceChildren();const hero=node('header','','mp-hero'),copy=node('div','','mp-hero-copy');copy.append(node('span','YATSUN · MULTIPLAYER','mp-eyebrow'),node('h1',label),node('p',root===mpJoin?'En ledig plats. En ny motståndare. Slå dig ner.':'Ett kast nu. Returmatch imorgon. Spela tillsammans, i er egen takt.'));const art=node('div','⚄','mp-hero-die');art.setAttribute('aria-hidden','true');hero.append(copy,art);if(root===mpJoin){const back=button('← Multiplayer',openLobby);back.className='mp-back';root.append(back);}root.append(hero);const status=node('p','','mp-status');status.setAttribute('role','status');root.append(status);}
