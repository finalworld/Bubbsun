...
export async function loadRecipes(groupId:string):Promise<Recipe[]>{const snap=await getDocs(query(collection(db,"groups",groupId,"recipes"),orderBy("updatedAt","desc")));return snap.docs.map(parseRecipe)}
...
export async function loadRecipeViewCount(recipe:Recipe,creatorUid:string):Promise<number>{if(!creatorUid||creatorUid!==recipe.creatorId)return 0;const snapshot=await getDocs(query(collection(db,"recipeViews",recipeViewKey(recipe),"viewers"),where("creatorId","==",creatorUid)));return snapshot.size}
...
export function watchAdminUserCounts(userIds:string[],callback:(counts:Record<string,AdminUserCounts>)=>void):Unsubscribe {
  ...
  type CountEntry={uid:string;key:CountKey;identity?:string};
  const values=new Map<string,CountEntry[]>(),unsubs:Unsubscribe[]=[],groupUnsubs:Unsubscribe[]=[];
  const emit=()=>{const result:Record<string,AdminUserCounts>=Object.fromEntries(userIds.map(uid=>[uid,{notes:0,calendarEvents:0,recipes:0,groups:0,followedLists:0}])),seenRecipes=new Set<string>();for(const entries of values.values())for(const entry of entries){result[entry.uid]??={notes:0,calendarEvents:0,recipes:0,groups:0,followedLists:0};if(entry.key==="recipes"&&entry.identity){const identity=`${entry.uid}:${entry.identity}`;if(seenRecipes.has(identity))continue;seenRecipes.add(identity)}result[entry.uid][entry.key]++}callback(result)};
  const set=(source:string,entries:CountEntry[])=>{values.set(source,entries.filter(entry=>entry.uid));emit()};
  ...
  for(const uid of userIds){
    ...
    unsubs.push(onSnapshot(collection(db,"users",uid,"privateRecipes"),snap=>set(`privateRecipes:${uid}`,snap.docs.map(item=>({uid,key:"recipes" as const,identity:item.id})))));
  ...
  unsubs.push(onSnapshot(collection(db,"groups"),groups=>{
    ...
    for(const group of groups.docs){
      groupUnsubs.push(onSnapshot(
        collection(group.ref,"members"),
        snap=>set(`groupMembers:${group.id}`,snap.docs.map(item=>({uid:item.id,key:"groups" as const}))),
      ));
      groupUnsubs.push(onSnapshot(
      ...
      groupUnsubs.push(onSnapshot(
        ...
        snap=>set(`groupRecipes:${group.id}`,snap.docs.map(item=>({uid:textValue(item.data().creatorId),key:"recipes" as const,identity:item.id}))),
      ));
    }
    emit();
  }));
